...
export default defineConfig({
  testDir: './tests/ux', testMatch: process.env.GI_UX_STEER ? ['queue-steer.spec.mjs'] : ['classic.spec.mjs', 'session.spec.mjs', 'drafts.spec.mjs', 'queue.spec.mjs', 'models.spec.mjs', 'context.spec.mjs', 'queue-return.spec.mjs'], timeout: 30000,
  expect: { timeout: 4000 }, retries: 0, workers: 1,
  outputDir: 'test-results/ux-parity/artifacts',
  reporter: [['line'], ['json', { outputFile: process.env.GI_UX_STEER ? 'test-results/ux-parity/steer-results.json' : 'test-results/ux-parity/results.json' }]],
  use: {
