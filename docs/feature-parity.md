...
| Copy and links | `/copy` for latest assistant source, fullscreen drag/copy/edge scroll, word/line multiclick for eligible text, opt-in native/OSC52 clipboard, safe OSC8 targets for supported intact links. | Host clipboard/URL activation depends on the terminal. Wrapped/complex links, cross-wrap word selection and locale-specific segmentation parity are incomplete. |
