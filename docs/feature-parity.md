...
native auth persistence repairs. Multi-passkey enrolment, remote networking and
MCP are requirements with implementation work outstanding.
...
| Multi-passkey management | Planned under the separately pinned Piclaw contract. | Native Go WebAuthn verification, session-bound five-minute proof, one-use challenges, two-key independent sign-in after restart and concurrent last-key protection. Virtual and physical authenticator results reported separately. |
...
CI has no Playwright gate yet. Remaining priorities include the fresh-chat/Return
