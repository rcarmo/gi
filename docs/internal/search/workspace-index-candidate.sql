-- Candidate only: not run by Gi database initialisation.
-- Versioned shadow tables permit validation before retiring the old scaffold.
-- The caller must enable foreign_keys on every connection before a transaction.
