...
3. **Storage API implemented by ADR-0043:** stable unchanged chunk/document IDs, transactional FTS maintenance, scope membership cleanup and rollback on incomplete/invalid snapshots. A real scanner must still establish complete-inventory evidence before calling it.
4. **Storage protocol implemented by ADR-0043:** per-workspace token/expiry fences, renewal, failure reporting and stale projection/takeover recovery, tested with two stores. The worker, periodic renewal, cleanup scheduling and process-crash acceptance are not yet wired.
...
6. Expose real status/reindex and bounded lexical query results to web, tools and TUI. Ordinary reads never block on refresh unless explicitly requested. Keep semantic capability absent until embeddings/vector storage/query paths are independently verified.
