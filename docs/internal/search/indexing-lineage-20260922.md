...
5. Adapt Piclaw's incremental metadata fast path, with explicit full/hash verification for metadata-collision cases. **ADR-0048 supplies durable invalidation capture/ack revisions**, including events during commit; **ADR-0049 supplies an internal bounded coalescing scheduler** with revision-aware completion, contention retries and cancel/join shutdown. Application ownership and watcher/mutation delivery still need wiring.
