...
The pinned feature corpus covers speech playback, not this voice-capture event contract; no new formal feature mapping is claimed. The later [local-notification slice](local-notifications.md) adds open-tab notification contracts; Web Push, real audio/OS prompt/device acceptance and Visual remain open. TUI adaptation is terminal/OS dictation as ordinary paste/input; no terminal microphone indicator or idle row is added. Voice-input CI36202895941 passed after run36201925678 was cancelled. The combined runtime through `dcc8c16` is [deployed with local notifications](local-notifications.md) after CI36207372088. Read-only live capability checks did not activate recognition or request microphone permission; real audio/prompt acceptance is still unverified.
