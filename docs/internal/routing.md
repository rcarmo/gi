...
- the guarded web runtime layer now exposes that narrow path via `/api/runtime/inbound-work` and `/api/runtime/inbound-work/drain`, giving the queue a real runtime ingress/worker-style caller; the list route now also reports per-status counts and supports `eligible=true|false` so callers can distinguish runnable work from retry/backoff rows
