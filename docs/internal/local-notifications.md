...
Voice-input commit `7f5f26f` passed replacement CI36202895941 after CI36201925678 was cancelled. Runtime `dcc8c16` passed replacement CI36207372088 and is deployed from an isolated checkout on8090, PID553455/PGID553342. Six guarded Chromium/WebKit×viewport probes pass capability visibility/default-off opt-in and prior panel/focus/draft behaviours without requesting permissions, sending prompts or any HTTP writes/page errors. DB62/51/146 and auth state remain unchanged apart from runtime leases; integrity/FKs pass. The isolated build regenerated assets from the green source; the unverified CI-only follow-up was not included in runtime source. Run36206444743 passed notification, passkey and core jobs but the expanded browser-journey job exceeded its15-minute limit (check annotation108303890554). The four compose/panel/voice targets now run in a separate required15-minute job; the original journey, geometry, slash and workspace targets remain in their existing required job. All targets and artifact uploads are retained, with no timeout increase or test skip.
