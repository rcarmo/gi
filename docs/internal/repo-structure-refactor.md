...
- the same consolidation then removed the row-returning allocation-resolution wrappers too: `ResolveSessionByAllocation(...)`/row-returning `FindSessionByAllocation(...)` were collapsed into the ID-first allocation finder surface, and runtime/tests now explicitly load the final session row only when they truly need payload/state data
- routed turn-resolution plumbing was then consolidated the same way: routed resolution state and route-session APIs now carry source/target session ids instead of `*store.Session` objects after inbound-context preparation, and peer-route/orchestration call sites were updated directly to the ID-based signatures instead of keeping a row-centric compatibility layer
- routing-specific parsing started moving out of `internal/turn` into `internal/routing`: directed-prompt parsing now lives under `internal/routing`, routed prompt/peer preparation now builds route context from session ids plus store-backed identity instead of loading source session rows just to derive inbound routing context, the old exported peer-session wrapper that returned `*store.Session`/`created` was narrowed to a simple session-id API for the TUI `/fork` path, the temporary `routeSessionPlan` struct was removed so route-session lookup/clone helpers now take explicit `sourceSessionID`/`route`/`allocation` arguments, the temporary `routedPromptResolution` struct was removed so routed prompt/peer preparation now uses direct return values instead of mutating a shuttle object across helpers, the trivial `resolveRoutedPromptTarget` forwarding helper was deleted so callers invoke `ResolveOrCreateRouteSession` directly, the one-use `applyLocalRouteMetadata` helper was inlined back into the same-session routed prompt path, the one-use `copyRouteSessionHistory` helper was inlined into `cloneRouteSession` so the clone path reads straight through, the thin `preparePeerRouteResolution` helper was deleted so the peer-message and peer-session callers now build their route context inline, the thin `preparePromptRouteResolution` helper was deleted so the routed prompt entrypoint now performs prompt parsing, inbound-context building, and mention override handling inline, the one-line `inboundContextFromSessionID` wrapper was removed so callers use the fallback-aware helper directly, the tiny `splitScopedValue` parser helper was inlined into the inbound-context builder so scope decoding no longer bounces through a separate local helper, the inbound-context helper itself now takes a single explicit context so callers choose the exact durability/ownership context they want instead of threading a second fallback parameter through the API, route-audit publishing was flattened to `internal/routing/audit.go` while persistence now lives in `internal/store/audit/routing_events.go`, and routed-session allocation/context ownership now lives under `internal/routing/routedsession` instead of `internal/turn`
- `internal/turn` routing-facing engine methods were split out of `engine.go` into:
  - `routing_runtime.go`
- `internal/turn` generic helper ownership was tightened further: turn-phase/subturn delivery normalization moved to `internal/store/turn_helpers.go`, turn-failure runtime marking moved to `internal/store/turn_failures_runtime.go`, and the shell runtime executor moved to `internal/tools/shell_runtime.go`; routing/runtime/direct/steering/recovery/hook state/process/default-tools and agent-loop phase helpers were then folded back into `internal/turn/engine.go` to reduce file scatter and keep critical coordination logic discoverable
- follow-up store regrouping carved out functional subpackages: `internal/store/audit` (routing + hook audit persistence), `internal/store/queue` (inbound queue + dispatcher lease), `internal/store/cache` (ACME caches), `internal/store/object` (kv/blob helpers), `internal/store/vfs` (path/namespace/virtual helpers), and `internal/store/internalx` (shared coercion/string helpers), with callsites moved to direct package ownership APIs instead of keeping thin forwarding shims
- `internal/tui` session-reference, agent-tree, and fork-target helpers were split out of `chat.go` into `session_refs.go`
- tool/extension ownership moved out of `internal/turn`: tool registry/scope now live under `internal/tools/registry.go` and `internal/tools/scope.go`, extension discovery/loading now lives under `internal/tools/extensions.go`, and skills tool listing/lookup moved to `internal/skills/tools_tool.go`
- process-hook runtime and hook-state/script-response helpers were consolidated under hook ownership (now co-located with hook registry/runtime), while turn topics/recovery/steering/direct runtime slices were consolidated under engine ownership
