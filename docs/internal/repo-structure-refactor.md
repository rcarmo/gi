...
- `internal/turn` routing-facing engine methods were split out of `engine.go` into:
  - `routing_runtime.go`
- `internal/turn` generic helper ownership was tightened further: generic value coercion moved to `internal/store/value_helpers.go`, turn-phase/subturn delivery normalization moved to `internal/store/turn_helpers.go`, turn-failure runtime marking moved to `internal/store/turn_failures_runtime.go`, and the shell runtime executor moved to `internal/tools/shell_runtime.go`; routing/runtime/direct/steering/recovery/hook state/process/default-tools helpers were then folded back into `internal/turn/engine.go` / `internal/turn/agent_loop.go` to reduce file scatter and keep critical coordination logic discoverable
