...
[ADR-0054](../adr/0054-native-quick-actions.md) verifies browser timeline typeahead and command prefill only. Terminal ordinary typing must remain editor input. Reuse explicit Alt-S/Alt-M/Alt-I and slash commands; any combined chooser needs an explicit trigger and at most six temporary results, separate insert/run semantics, Escape draft/cursor/reader restoration and zero idle rows. Session ownership, failure and three-size fullscreen/regular acceptance are required before terminal credit. No global printable-key interception or permanent palette row.
...
## Latest-assistant source copy (2026-09-23; verified subset)

The existing `/copy` now sends the latest nonempty assistant message's **exact stored bytes** via native or OSC 52 opt-in, retaining leading/trailing whitespace and Unicode. Clipboard-off or missing-helper fallback uses the existing transcript, not a false success. Native six-size fullscreen/regular PTYs validate emitted OSC 52 bytes, fallback, unchanged draft and zero added idle rows; unit tests cover native helper bytes and failure. [ADR-0058](../adr/0058-terminal-latest-assistant-source-copy.md). Fullscreen selection still copies rendered cells and visual-row breaks. A per-message source action needs a stable message ID, explicit shortcut or bounded selector, origin/focus guards and independent three-size evidence. Browser [ADR-0055](../adr/0055-message-copy-clipboard-safety.md) and shared [ADR-0057](../adr/0057-integrated-message-copy-delete.md) do not establish terminal per-message or deletion parity.
