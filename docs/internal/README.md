...
- `internal/turn` routing/helper/shell/value logic has been split out of `engine.go`, the former turn-local session-identity projection helpers were moved back into `internal/store`, routed resolution/signature handling now prefers session-id transport over carrying full `*store.Session` objects after route context preparation, directed-prompt parsing now lives in `internal/routing` instead of `internal/turn`, the exported peer-session API used by the TUI was narrowed to return only a target session id instead of a full session row plus creation flag, route-session helpers now take explicit inputs instead of a temporary `routeSessionPlan` wrapper struct, routed prompt/peer preparation no longer uses a mutable `routedPromptResolution` shuttle struct, direct routed callers now invoke `ResolveOrCreateRouteSession` instead of going through a redundant forwarding helper, same-session routed prompt metadata is now written directly at the call site instead of going through a single-use helper, clone-time route-session history copying is now inline inside `cloneRouteSession` instead of hidden behind another one-use helper, peer-message/peer-session route preparation is now inline at the call sites instead of hidden behind a thin helper, routed prompt preparation is likewise inline in `SubmitPromptRouted` instead of hidden behind another thin helper, callers now use `inboundContextFromSessionIDWithFallback` directly instead of a redundant one-line wrapper, scope-value parsing is now inline inside the inbound-context builder instead of hidden behind a tiny local parser helper, the inbound-context helper now takes a single explicit context so callers decide directly whether they want active caller semantics or engine-owned durable semantics, and the remaining routed-session allocation/context code plus route-audit publication code now live in dedicated `internal/turn/routedsession` and `internal/turn/routeaudit` subpackages instead of root-level `internal/turn` files
