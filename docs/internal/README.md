...
- the first durable inbound queue layer is now live via `inbound_work_queue` plus engine enqueue/claim/process/drain helpers; the guarded web runtime now exposes enqueue/list/drain endpoints and starts a small configurable background dispatcher without inventing a parallel runtime path
