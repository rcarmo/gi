...
- `internal/turn` routing/helper ownership was tightened: directed-prompt parsing now lives in `internal/routing`, routed session allocation/context now lives in `internal/routing/routedsession`, route-audit publication is flattened at `internal/routing/audit.go`, and routing-event persistence remains in `internal/store/routing_events.go`; the exported peer-session API used by the TUI now returns only a target session id (no row wrapper), route-session helpers take explicit inputs (no `routeSessionPlan` wrapper), routed prompt/peer preparation no longer uses a mutable `routedPromptResolution` shuttle struct, same-session route metadata/history helpers were inlined, and inbound scope parsing is inline in the inbound-context builder with a single explicit context
...
- generic runtime helper ownership tightened: value coercion and turn-phase/subturn-delivery normalization moved into `internal/store`, and turn-failure runtime marking moved into `internal/store/turn_failures_runtime.go`
