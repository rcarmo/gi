...
- the TUI now has a first topic-native consumption slice for canonical `runtime.*` topics on the active session, using `runtime.tool` / `runtime.hook` / `runtime.turn` / `runtime.session` notices for status and transcript updates alongside the legacy broadcast channel
