...
- the first durable inbound queue layer is now live via `inbound_work_queue` plus engine enqueue/claim/process/drain helpers; the guarded web runtime now exposes enqueue/list/drain endpoints, eligibility-aware queue introspection, and a small configurable background dispatcher with bounded retry/backoff state, without inventing a parallel runtime path
