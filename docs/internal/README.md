...
- hook invocation lifecycle plus higher-level hook decision notices now publish under `runtime.hook`, and hook invocation audit persistence, turn-failure persistence, and turn finalization persistence now use engine-owned background context where they must outlive request cancellation without becoming process-lifetime work
