...
- dedicated topic SSE streaming is now live via `/sse/topics`, with monotonic envelope sequences exposed as SSE `id:` plus reconnect gap metadata on `connected`
- script-facing topic APIs now exist in both script bridges: JS exposes `gi.topics.publish(...)` plus polling-style `gi.topics.subscribe(...)`, `gi.topics.read(...)`, and `gi.topics.unsubscribe(...)`, while embedded Joker exposes `gi-topic-publish`, `gi-topic-subscribe`, `gi-topic-read`, and `gi-topic-unsubscribe`; topic reads now include the bus-wide `sequence`, closed topic subscriptions are dropped eagerly on read, explicit unsubscribe stays idempotent, and raw/websocket close operations follow the same idempotent cleanup contract
