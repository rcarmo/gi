...
- `internal/turn` ownership was tightened and de-fragmented: routed session allocation/context now lives in `internal/routing/routedsession`, route-audit publication is flattened at `internal/routing/audit.go`, routing-event persistence remains in `internal/store/routing_events.go`, and most runtime split files were folded back into `engine.go` / `agent_loop.go` so turn coordination logic is centralized instead of spread across many micro-files
