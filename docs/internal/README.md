...
- `internal/turn` routing/helper/shell/value logic has been split out of `engine.go`, the former turn-local session-identity projection helpers were moved back into `internal/store`, routed resolution/signature handling now prefers session-id transport over carrying full `*store.Session` objects after route context preparation, directed-prompt parsing now lives in `internal/routing` instead of `internal/turn`, and the exported peer-session API used by the TUI was narrowed to return only a target session id instead of a full session row plus creation flag
