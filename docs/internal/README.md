...
- hook invocation lifecycle plus higher-level hook decision notices now publish under `runtime.hook`, and both hook invocation audit persistence and turn-failure persistence now use engine-owned background context where they must outlive request cancellation without becoming process-lifetime work
