...
- the first durable inbound queue layer is now live via `inbound_work_queue` plus engine enqueue/claim/process/drain helpers, and a guarded web runtime caller now exposes that path for enqueue/list/drain without inventing a parallel runtime path
