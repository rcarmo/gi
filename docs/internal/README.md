...
- the first durable inbound queue layer is now live via `inbound_work_queue` plus engine enqueue/claim/process/drain helpers, so direct/IPC/system callers can stage normalized ingress durably without inventing a parallel runtime path
