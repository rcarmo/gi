...
- hook invocation lifecycle plus higher-level hook decision notices now publish under `runtime.hook`, and hook invocation audit persistence now uses the engine background context so it survives request cancellation without becoming process-lifetime work
