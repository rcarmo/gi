...
Latest terminal link slice (2026-09-24): safe OSC8 metadata on intact parenthesised HTTP(S) targets survives fullscreen search; stationary linked tool-body clicks do not toggle blocks, while drag copies visible text.6link PTYs+9selection/search/regular regressions/98standardfunctional/96helpers/Go-vet-hook/TUI race x3 pass. No idle rows or browser mapping change: **96/236 Classic + 30/42 shared**, **140/12** unmapped. Details and limits below.

Earlier Settings acceptance (2026-09-24): Settings002 independently verifies uncached immediate shell and first General resolution, with held native reads,503/retry and exact draft/media retention.216Settings browser/98standard functional/96helpers with1,175 assertions/Go-vet-hook/review pass. Coverage **96/236 Classic + 30/42 shared**, **140/12** unmapped. Test-only; no restart or terminal behaviour change. Details below.
...
## Terminal links without new chrome (2026-09-24)

Gi's Markdown projector emitted plain label/URL text despite go-tui's OSC8 support. `transcriptLinkSpans` now decorates intact parenthesised HTTP(S) URLs with `TextSpan.Link`, preserving the exact visible text and wrapping. It rejects credentials, missing hosts, backslashes, control characters (including percent-decoded controls) and targets over2048bytes. Inline-code spans are left alone. Complex destinations containing parentheses and URLs split across lines remain ordinary text rather than guessed links. There is no shell/browser launcher, automatic fetch, or new setting; terminal-client modified-click support determines activation.

Rendered search rows retain each grapheme cell's link target. Selection hit-testing uses display columns, including both cells of wide characters. A stationary press/release on linked text does not also toggle its containing tool block; ordinary block clicks still work, and dragging copies only the visible selected cells. Regular mode retains native terminal selection and emits no mouse capture. The tests prove OSC8 emission, not physical browser opening by every terminal client.

Six native PTYs at60x18,100x22,140x36 in fullscreen/regular modes verify emitted targets, fullscreen search preservation, SGR stationary/drag actions, exact partial-link OSC52 text, draft/cursor/resize retention and unchanged idle footprint. Unit/race tests cover URL rejection, wide-cell hit testing and linked versus ordinary tool-body clicks. Nine existing selection/search/regular PTYs,98standard functional tests,96helpers with1,175 assertions and Go/vet/hook pass; the2seed-dependent browser functional cases are not counted. Older PTY scripts now receive `GI_TUI_BIN` from Make so an isolated build cannot overwrite the live `bin/gi` symlink; the initial attempt failed safely with text-file-busy before running tests.

Review's whole-span-copy concern does not apply to the grapheme-cell rows used by selection; the exact partial-link copy test confirms that boundary. Half-open right-edge selection and headless native-copy completion are pre-existing behaviours outside this patch. Double-click word selection, complete wrapped-link spans and those selection edges remain follow-up work. Browser coverage stays96/236 Classic and30/42 shared.

Deployment: `3a060d7` pushed and restarted through the standard Make target on8090, PID1698461. Mutation-blocked six-browser-size existing-message/status/draft smoke reported zero API writes/page errors. SQLite integrity is `ok`;62sessions,51turns,146messages unchanged. Six native terminal captures and regression logs are attached as `/workspace/tmp/gi-tui-links-evidence.tar.gz`.

## Fullscreen selection at the right edge

A drag ending at the final visible text cell now includes that cell. The selection snapshot stores content width separately from viewport width: go-tui reports a viewport including its scrollbar, although child layout reserves that column. Initial scrollbar presses still pass through; an existing text drag can reach the final cell or clamp beyond it. The final text cell maps to the outer half-open boundary in either drag direction. Interior endpoints keep their existing convention, and stationary linked clicks use the actual final content cell for link lookup.

`make test-terminal-links` passes race x3 with ASCII, wide and combining suffixes, forward/reverse copy, stationary clicks, interior endpoints and out-of-window clamping at widths 60/100/140, with and without vertical overflow. `make test-tui-selection-edge BIN_DIR=/tmp/gi-media-bin` passes six native PTYs at 60x18, 100x22 and 140x36: persisted system rows fill the content width; fresh OSC52 sentinels prove each drag direction copies the final grapheme; scrollbar-origin drags into text produce no copy. Draft text, cursor, resize and idle separator positions survive, with zero submitted turns. Captures and results are under `test-results/tui-selection-edge/`.

Fifteen existing selection/search/link/regular PTYs, `make test vet bun-checks`, 98 standard functional tests (two seed-dependent skips), and 96 helper tests with 1,175 assertions pass. Review prompted independent reverse OSC52 assertions in the unit test as well as the PTY. Regular mode still uses terminal-owned selection and its five-row idle dock. No browser mapping changed: 96/236 Classic and 30/42 shared, leaving 140 and 12 unmapped. The outer-boundary snap does not provide precise selection of only the final single-width cell; word selection and wrapped/complex links also need separate work.

Deployment: `122f5ba` is running on port 8090 as PID 1731562 after `make restart BIN_DIR=/tmp/gi-scheduler-bin BIND=0.0.0.0 PORT=8090`. Six guarded Chromium/WebKit phone/tablet/desktop checks preserve the composer draft and existing tool status, with zero API mutations or page errors. Read-only SQLite integrity is `ok`; counts before restart and after the browser checks match at 62 sessions, 51 turns and 146 messages. These live web checks cover deployment health; native selection results come from the isolated PTYs above.

## Non-iPad lightbox boundary

Native image clicks and trusted touch taps open the supplied lightbox on non-iPad runtimes across all six browser projects. The added Gi-only regression checks actual user agent/platform/touch metadata without overriding detection, exact stored image bytes and dimensions, absence of annotation controls, unchanged session/message history, and exact IndexedDB draft/media bytes through dismissal and reload. No API mutation occurs during activation.

Frozen timeline-006 stays unmapped. `Post.handleImageClick` unconditionally opens the lightbox, and Gi has no iPad-positive annotator path. The non-iPad result alone cannot prove the final device-gating clause. The test attaches the frozen steps for traceability but omits the frozen ID from its title; the reporter and source assertions are unchanged. iPad drawing, cropping and annotation persistence need their own implementation and tests.

Sixty lightbox/rendering browser tests, 98 standard functional tests (two seed-dependent skips), 96 helpers with 1,175 assertions, Go tests, vet and Bun checks pass. A first test attempt used the wrong IndexedDB record shape, which was corrected to the native `draft.media` record. Parallel builds later collided in shared generated assets and produced blank-page failures; both suites passed on clean sequential reruns. This test-only change needs no restart. The existing port-8090 process is PID 1731562; read-only SQLite integrity is `ok`, with 62 sessions, 51 turns and 146 messages. Browser mappings remain 96/236 Classic and 30/42 shared.

Terminal adaptation remains text-only attachment references with terminal-owned selection in regular mode. No image canvas, browser launcher or idle control is added. Model-facing message retrieval was also inspected: Gi has session-local UUID message listing but no public explicit row-ID/context-window tool contract, so original-025/shared38 remain unmapped.

## Per-occurrence terminal search

Fullscreen search now counts each non-overlapping occurrence within a rendered row. Enter, Shift-Enter and the existing Ctrl-G navigation move between occurrences on the same row before advancing to another row. The active match uses Pi's existing foreground/background colours; other matches are underlined. Unmatched cells retain their original styles, and all cells retain OSC8 link metadata. The search input and status label keep their existing footprint.

Each lowercase UTF-8 byte maps back to its original rendered grapheme's display-cell interval. This handles byte-length changes such as `İ` to `i`, wide characters and combining suffixes without splitting a visible glyph. Matching uses simple lowercase, not full Unicode case folding or normalisation. Trailing row padding is excluded as before, and matches never cross rendered rows. Refresh retains the selected row/column when appending output leaves that position intact; reflow, eviction and mutation-stable identity anchors need separate work.

`make test-terminal-links` passes race x3 with precise active/inactive cell styling, repeated Unicode matches, link targets inside/outside highlighted ranges, non-overlap, padding exclusion, append retention and navigation wrap. Three `make test-tui-search BIN_DIR=/tmp/gi-media-bin` PTYs at 60x18, 100x22 and 140x36 navigate 96 occurrences, compare same-row ANSI changes without text changes, exercise reverse/forward wrap and preserve draft, cursor, reader position, resize and hidden-tool behaviour. Eighteen edge-selection, selection, link and regular-mode PTYs also pass. The link test now searches inside the URL to force linked-cell redraw; precise highlighting correctly leaves an unmatched URL untouched, so the previous raw-output expectation was no longer valid.

Go tests, vet, Bun checks, 96 helpers with 1,175 assertions and 98 standard functional tests (two seed-dependent skips) pass. Review found no blocking mapping issue under the row-local/simple-lowercase contract. Regular mode keeps terminal-owned history/search and its five-row dock. Browser mappings remain 96/236 Classic and 30/42 shared.

Deployment: `e3fa24e` is running on port 8090 as PID 1804143 after `make restart BIN_DIR=/tmp/gi-scheduler-bin BIND=0.0.0.0 PORT=8090`. Six guarded Chromium/WebKit phone/tablet/desktop smoke checks preserve the draft and tool-status identity/duration, with zero API writes or page errors. SQLite integrity is `ok`; counts before restart and after the checks match at 62 sessions, 51 turns and 146 messages. Native search behaviour is verified by the separate PTYs above.

## Protected recovery control display

Frozen extra012 now has independent six-project coverage. The supplied Post is unchanged; a guarded build adapter imports `gi-recovery-control.ts` and suppresses a validated protected continuation after all hooks run. The validator comes from Piclaw `70d33bc93` (`runtime/web/src/components/post.ts`). It requires the control type/intent/schema, non-empty source message ID, positive row/thread/depth values and valid typed handoff fields. Primary-failure metadata is validated even on legacy-shaped blocks: review found that the upstream early return otherwise hid malformed timeout fields. Invalid reason/compaction/tool-required combinations stay visible.

The isolated native server inserts 23 variants using `Store.AddMessage`, including legacy/typed/timeout controls, malformed base fields, partial typed/failure fields and semantic-invalid combinations. Browser tests verify the native payloads, hidden valid rows and visible malformed neighbours in the timeline, reload and actual native search results. Stored messages, session identity, exact IndexedDB draft/media bytes and the empty turn list remain unchanged; no API writes occur during the checks. Five-project reporter input remains partial; all six projects are required for a pass.

Sixty recovery/copy/delete and 24 recovery/speech browser cases pass, alongside the seeded functional test, 98 standard functional tests (three fixture-dependent skips), 98 helpers with 1,243 assertions, Go tests, vet and Bun checks. Initial fixture integration mistakes were corrected to Gi's actual store API, `source_row_id`/`thread_id` and typed field names, nullable empty-turn response and `messages` search envelope. No frozen feature, supplied component or reporter rule was weakened. Coverage is 97/236 Classic and 30/42 shared, leaving 139 and 12 unmapped. Empty informational recovery placeholders (extra013) remain separate.

This change grants no control authority and never removes stored history. Gi's native recovery does not currently emit these imported control-intent rows; it uses turn events and final-reply recovery markers. Terminal history therefore stays unchanged. A future terminal filter must validate structured stored metadata before rendering, preserve malformed authored text, and add no idle rows; browser results provide no terminal credit.

Deployment: `5d90ad3` is running on port 8090 as PID 1846274 after `make restart BIN_DIR=/tmp/gi-scheduler-bin BIND=0.0.0.0 PORT=8090`. Six guarded Chromium/WebKit phone/tablet/desktop smoke checks preserve the draft and existing tool status, with zero API writes or page errors. SQLite integrity is `ok`; counts before restart and after the checks match at 62 sessions, 51 turns and 146 messages. No recovery-control fixtures were added to the live database; positive suppression is covered by the isolated native fixture above.

## Empty informational recovery placeholders

Frozen extra013 now has independent six-project coverage. The guarded Post adapter omits an agent response only when the first `turn_outcome_marker` has kind `recovery` and severity `info`, the supplied `shouldRenderContent` is false, and no visible extras remain. This is Piclaw `70d33bc93`'s placeholder representation. Warning/error rows, other marker kinds, user messages, authored text and whitespace stay visible. Media, cards, submissions and reference-only posts also stay visible; the adapter conservatively retains resources, links, widgets and annotations. All hooks still run in the same order before suppression.

Twenty-one native stored variants cover the empty positive and text, warning/error, metadata, first-marker precedence, real file bytes, rendered cards and fallback cards, submission receipts, file/message/attachment references, expanded embedded resources and annotations. Checks preserve exact IndexedDB draft/media bytes, session identity and stored messages through reload and search. The search check covers authored text; the empty row remains in storage without gaining rendered content. No API writes occur during these checks.

Real card preservation exposed a missing SDK and static route. Gi now serves the supplied renderer's `/static/js/vendor/adaptivecards.min.js` from the pinned Piclaw asset: Adaptive Cards 3.0.6, SHA-256 `c3404b6cba966546c03ab9f81b133a2b8d9185da29806457d0adb92c107f799c`, with upstream metadata and MIT licence. A Go test verifies GET/HEAD, JavaScript MIME and the exact bytes. This restores static rendering; native card action mutation and the broader card scenarios are not implemented by this slice.

Thirty recovery/speech and 102 recovery/copy/delete/lightbox browser cases pass, along with two seeded functional tests, 98 standard functional tests (four fixture-dependent skips), 100 helpers/1,293 assertions, Go tests, vet and Bun checks. Review found no runtime blocker and prompted additional reference/resource/annotation cases plus a stronger duplicate-adapter guard. Early runs found an adapter scope error and the missing SDK route; later fixture corrections used the real submission title, reference selector and collapsed-resource behaviour. Frozen features, supplied components and report completeness gates remain unchanged.

Coverage is 98/236 Classic and 30/42 shared, leaving 138 and 12 unmapped. Native Gi recovery still uses turn events and final-reply markers, not these imported placeholder blocks. Terminal history is unchanged; any future terminal suppression must use validated structured metadata and preserve meaningful text/media references without adding idle rows. The browser result grants no new TUI credit.

Deployment: `19c01a1` is running on port 8090 as PID 1904551 after `make restart BIN_DIR=/tmp/gi-scheduler-bin BIND=0.0.0.0 PORT=8090`. Six guarded Chromium/WebKit phone/tablet/desktop checks preserve drafts and existing tool status, with zero API writes or page errors. SDK GET bytes match the pinned SHA-256; HEAD returns 200 and JavaScript MIME. SQLite integrity is `ok`; counts before restart and after the checks match at 62 sessions, 51 turns and 146 messages. Positive placeholder/card tests used only isolated fixtures, never the live database.

## Unsupported Adaptive Card submission

Frozen extra003 now has independent six-project coverage. Gi can render cards, but it has no native action identity/authorisation/persistence contract. `submitAdaptiveCardAction` therefore rejects asynchronously with an explicit unsupported/not-submitted error. The previous `null` result incorrectly resolved the action. The supplied renderer catches the rejected Promise, shows its existing error notice, clears busy styling and keeps inputs enabled. No supplied component or SDK source was changed.

An isolated native fixture stores an active card with a text input, checkbox and Submit action in two sessions. Real pointer and keyboard SDK activation produce the error notice; retry creates a fresh notice and busy state settles. Card text/choice, exact IndexedDB composer/media bytes, native message history and the other session remain intact. There are no API writes, new turns, completion banners or submission receipts. Reload preserves the composer and stored active state; card edits are mount-local and are not presented as persisted submissions. The helper test verifies the async rejected-Promise boundary for arbitrary payloads. Five-project reporter input remains partial.

Thirty card/recovery/speech and 66 card/recovery/copy/delete browser cases pass, alongside two seeded functional tests, 98 standard functional tests (five fixture-dependent skips), 101 helpers/1,307 assertions, Go tests, vet and Bun checks. Independent review found no blocker for the asynchronous rejection contract. Synchronous callback failures, extra002 submission identity validation and accepted actions remain separate work.

Coverage is 99/236 Classic and 30/42 shared, leaving 137 and 12 unmapped. Terminal card actions remain unsupported; no modal, action row or idle indicator was added. A future terminal adaptation must use a bounded transient prompt only after a shared native submission contract exists, with the original draft preserved and errors kept distinct from accepted responses.

Deployment: `2734ec5` is running on port 8090 as PID 1930575 after `make restart BIN_DIR=/tmp/gi-scheduler-bin BIND=0.0.0.0 PORT=8090`. Six guarded Chromium/WebKit phone/tablet/desktop smoke checks preserve drafts and existing tool status, with zero API writes or page errors. SQLite integrity is `ok`; before/after counts match at 62 sessions, 51 turns and 146 messages. Card rejection was verified only with isolated native fixtures; no fixture rows were added to the live database.

## Compact terminal session actions

Alt-S keeps the existing six-result session picker. Right-arrow opens at most two actions for the selected session: Pin/Unpin and, when available, Archive/Restore. The submenu shows the human-readable target; Enter applies one action and returns to the picker, while Escape or Left returns without writing. Status/pinned labels appear before long IDs so results stay legible at 60 columns. Rename is not included; it needs a separate text-editing prompt.

`SessionDisplayCapabilities` reads archived/pinned state and running, queued and active-claim existence in one query. These are display hints; the existing transactional `MutateSession` remains authoritative. The submenu captures both target identity and source session generation. Stale ownership, concurrent archive and late queued work cannot mutate the wrong session. Conflict errors return to the picker for a capability refresh; storage failures say the action was not confirmed. Success updates only the selected label, preserving query, ordering and viewport without changing the active session or adding transcript output.

Regular mode reuses the temporary picker screen and retains its largest session-menu region until close, so replacing a tall list with a short submenu clears old rows. Closing restores the original inline dock and terminal-owned history. No idle rows, toolbar or new persistent status indicator are added.

Six action PTYs cover fullscreen/regular at 60x18, 100x22 and 140x36: native pin/unpin/archive/restore, root/busy gates, queued work arriving after submenu open, resize, Unicode draft/cursor preservation, nine-session tall-to-short clearing, unchanged turn/message counts and no global-settings writes. Six session-picker, six model-picker and three regular-mode PTYs also pass. The regular harness now uses a private tmux socket for each launch, including reopen; the model harness uses per-case sockets and bounded resize settlement instead of racing server shutdown or terminal redraw. Initial harness failures were isolated and corrected before these runs.

Focused session-action/capability/picker race tests pass three times, along with Go tests, vet, Bun checks, 98 standard functional tests (five fixture-dependent skips), 101 helpers/1,307 assertions and all five Linux/macOS/Windows cross-builds. Final independent review found no blocker. Browser feature mappings remain 99/236 Classic and 30/42 shared; terminal rename and durable queue failure/retry acceptance remain separate work.

The session-action shipment's first CI run (`36048713987`) exposed an existing active-steering test race: its bootstrap shell could finish before the second prompt was submitted. The test now holds the real setup boundary, asserts same-turn steering while active, then releases and joins runner cleanup. A private database avoids stale runners between repetitions. All original admission/completion assertions remain; full Go/vet and 50 race-detector repetitions pass. `make test-active-steering` is included in CI. This correction changes no runtime behaviour.

Deployment: session actions `08933f4` and test stabilisation `8380098` are running on port 8090 as PID 2069584 after `make restart BIN_DIR=/tmp/gi-scheduler-bin BIND=0.0.0.0 PORT=8090`. GitHub CI run `36049515490` completed successfully: tests, shell-cancellation/active-steering race checks and all five Linux/macOS/Windows builds. Six guarded Chromium/WebKit phone/tablet/desktop smoke checks preserve drafts and tool status with zero API writes or page errors. SQLite integrity is `ok`; before/after counts match at 62 sessions, 51 turns and 146 messages. All terminal mutation tests used isolated databases, not live sessions.

## Terminal session rename

Alt-S, Right, Rename opens a four-row prompt containing its title/help, captured human-readable target, one horizontally windowed title row and validation/error feedback. The action submenu now has at most three choices. A separate editor buffer reuses existing movement, deletion, undo and yank operations; it never replaces the composer. Enter variants save, Ctrl-J cannot insert a newline, and Escape/Ctrl-C return to the action menu without writing. Mouse events cannot move the underlying composer while Rename owns the prompt.

The shared native mutation validates a trimmed 1–160-rune single-line title and rejects an archived target. The editor checks source session generation before writing; failed validation, archive races and storage errors retain the title for correction or cancellation. Editing clears stale validation feedback. Success refreshes the captured picker row and preserves its query/selection and the original composer text/cursor/undo/yank state. The single-row window uses terminal display widths and keeps the caret visible at narrow widths; movement remains the existing rune-based model, without a new grapheme-motion guarantee.

Six fullscreen/regular action PTYs at 60x18, 100x22 and 140x36 verify Unicode editing/undo, empty-title rejection and retry, native persistence, long-title single-row layout, resize and cancel without writes. Fifteen session-picker, model-picker and regular-mode PTYs pass. Focused rename/action/capability race tests pass three times, including stale scope, archive/storage failure and composer isolation. Go tests/vet, Bun checks, 98 functional tests (five fixture-dependent skips), 101 helpers/1,307 assertions and all five cross-build targets pass. Independent review found no blocker. No idle rows or web controls were added; browser mappings remain 99/236 Classic and 30/42 shared. Durable terminal queue actions remain open.

Deployment: `02bb43f` is running on port 8090 as PID 2122201 after `make restart BIN_DIR=/tmp/gi-scheduler-bin BIND=0.0.0.0 PORT=8090`. GitHub CI `36052708270` completed successfully: tests/race checks and all five builds. Six guarded Chromium/WebKit phone/tablet/desktop smoke checks preserve drafts and existing tool status with zero API writes or page errors. SQLite integrity is `ok`; before/after counts match at 62 sessions, 51 turns and 146 messages. Rename writes were exercised only in isolated PTY fixtures.

### Single-user browser TOTP bootstrap (2026-09-24)

The application now waits for a validated native `/api/auth/status` policy before mounting. Loading/error states expose no credential controls; Retry fetches fresh policy. An enrolled single-user instance offers only a six-digit code. `POST /api/auth/session` accepts bounded JSON with no username, preserves the existing native TOTP authority, and returns a host-only HttpOnly/SameSite Strict cookie rather than a JSON bearer token. Successful HTTP status alone is insufficient: the browser confirms cookie authority via status before mounting. Tokens/codes are not persisted in browser storage. Existing bearer/query clients retain precedence, including rejection of invalid explicit credentials rather than fallback to a valid cookie.

Browser credentials require **direct TLS**, or both loopback peer and loopback Host. Proxy headers are deliberately not trusted; TLS termination in front of plain remote HTTP is not supported by this transport. Cookie-authenticated reads/SSE and writes reject foreign Origin/Fetch-Site values. Protected HTTP responses and auth status use `private, no-store`. New requests revalidate expiry; already-open SSE expiry, session revocation/logout, rate limiting of the pre-existing TOTP endpoint and browser enrolment are not claimed here. The local TUI continues to use local store authority: no sign-in prompt, footer row, or passkey-management chrome is added.

Map only frozen `@ux-auth-002` and `@ux-auth-004`: code-only native admission and policy failure/Retry. Eighteen isolated auth cases prove code-only requests, invalid codes/network errors/pending locks, cookie invisibility to JavaScript, native API/SSE access, malformed-policy/false-success rejection, expiry/re-auth, and draft/media reload preservation. All six Chromium/WebKit phone/tablet/desktop projects pass. Authentication fixtures own their process/workspace and never alter the live auth file. Native tests additionally cover TLS cookie flags, explicit bearer precedence, restart token validation, hostile Origin/Fetch-Site, Host rebinding, proxy-header rejection, body limits and corrupt-state failure.

Validation: 264 shell/Settings/session browser regressions; 99 functional tests, 5 fixture-only skips; 103 helpers / 1321 assertions; Go tests/vet/hook; web/auth race ×3; five cross-builds. Two real first-paint regressions were fixed without relaxing assertions: leftover static loading markup and a deferred Settings-opening listener. One combined run was terminated by the tool timeout and grants no evidence; the complete unchanged rerun passed. Bounded independent design review found no blocker within the direct-TLS/localhost model; reverse-proxy support remains explicitly outside this slice. Coverage is now **101/236 Classic + 30/42 shared**, with **135/12** unmapped. CI/deployment evidence follows separately.

New upstream scope: Piclaw `b531ea3a8cb38ba8f1e0729f8b6d0439f63cb5f1` adds multiple single-user passkeys in both Settings skins. Its 26 scenarios/outlines are copied verbatim under `tests/ux/features/additions/piclaw-2026-09-24/`, hash-pinned separately from the frozen 236/42 inventory. Gi has no native WebAuthn credential/challenge implementation; all 26 remain unimplemented. Recent proof, owned rename/removal, concurrent lockout prevention, duplicate/uncertain registration and browser prompt/device evidence are prerequisites, not TOTP parity credit.

Deployment: `c96ecd4` is running on port 8090 as PID 2264426 after CI [36060715928](https://github.com/rcarmo/gi/actions/runs/36060715928) passed tests and all five builds. Six guarded Chromium/WebKit phone/tablet/desktop checks, repeated while inspecting DB drift, reported zero API writes/page errors and retained drafts/tool status. Live auth remains unenrolled: no live enrolment or login was performed. SQLite integrity is `ok`; counts remain 62 sessions, 51 turns and 146 messages. Full dumps differ because `kv_store/runtime_leases/inbound_dispatcher` renews normally; the repeated probe's dump diff contains only that row, and hashes excluding runtime-lease rows match. The deployment rebuild also refreshed the generated source map after removal of a trailing blank line in `gi-auth-policy.ts`; executable asset bytes are unchanged.

### Native auth-state transactions (2026-09-24)

Prerequisite work for future logout and passkey credential mutations. The old `auth.json` load/modify/direct-write sequence could lose concurrent sign-ins, publish partial JSON to readers, or let a stale enrolment candidate replace an enrolled account. All production enrolment/login/internal revocation writers now acquire the same nonblocking process-shared `auth.lock`, read inside the lock, and replace a complete bounded snapshot using a private same-directory temporary file. Linux/macOS use flock; Windows uses LockFileEx and Go's rooted replacement. Conflicts are retryable and surfaced as HTTP 409. No truncate-in-place fallback exists.

Unknown top-level JSON fields survive writes. Known fields cannot leak back through the extension map, notably the omitted empty session list after final-token revocation. `RevokeToken` removes only the captured token and is not exposed as an HTTP/browser logout operation. Lockless validations may finish using a pre-revocation snapshot already read; fresh requests after commit observe removal. Existing admitted streams are not revoked. The local filesystem owner remains trusted; bounded roots/type/identity checks detect unsafe paths and observed non-cooperating changes but do not claim protection from an adversarial local owner.

Durability limits: Linux/macOS sync the new file, state directory, and parent workspace directory (the last step was added after review caught the initial `.gi` creation gap). Windows flushes the temporary file and uses replacement semantics, but directory fsync is not portable here; **sudden-power-loss durability is not promised on Windows**. Native CI tests replacement, contention and process crash/restart there. Hardware power-cut testing is not claimed on any platform. A failed post-replace directory sync reports ambiguous durability, never a success token; subsequent status remains authoritative. Lock files persist and OS locks release on process exit.

Evidence: auth race ×10 with 24 concurrent login writers plus targeted revoke/read, four real subprocess writers, killed-holder release, single-winner/stale enrolment, native 409, final-token non-resurrection, private-file permissions, unknown-field preservation, corruption/oversize rejection, temp cleanup, and symlink/directory/lock-swap conflicts. Web/auth race ×3, 18 six-project browser auth regressions, 99 functional / 5 fixture-only skips, 103 helpers / 1321 assertions, full Go tests/vet/hook and five cross-builds pass. Added native Linux/macOS/Windows auth-state CI jobs, with builds depending on them. No browser criteria added: **101/236 Classic + 30/42 shared** remain; the 26 passkey scenarios and frozen logout notification cleanup still need implementation. No terminal UI changed.

### User correction: overall UX evidence was insufficient (2026-09-24)

The user reported broken Return-to-start, misaligned model/session pickers, wrong workspace-tab behaviour and a composer unlike Piclaw, then requested review of **all** UX tests. The source audit is recorded in `docs/internal/ux-test-audit-2026-09-24.md` with a 200-file CSV inventory. It covers all browser/functional/helper/TUI/feature/harness layers, not just another mapped slice. Read-only live comparison confirms component/layout drift and differing Escape focus behaviour. Exact Return-to-start and workspace transitions remain open reproductions. CI currently runs no browser UX suites; `make check` runs only functional Playwright. Some functional assertions are vacuous or cross-session-ambiguous; screenshots are mostly artifacts, not reference oracles. Strong bounded upload/draft/queue/SSE tests remain valid within their scope.

Treat 101/236 + 30/42 as the existing **source mapping**, not proof of current overall parity. Classic008's skill-prefill mapping is disputed against the frozen replacement semantics. No new credit or product fix is claimed by the audit. Pause passkey/auth expansion and prioritise actual composer/picker/slash/Quick Actions/Settings/workspace user journeys plus explicit all-suite/CI wiring. `56079fe` was not deployed because native macOS/Windows CI `36063764465` failed; live remains `c96ecd4`, port 8090 PID 2264426.

### Workspace animation repair (2026-09-25)

The reversed desktop close is now covered by a frame-level regression and repaired in `internal/web/static/css/gi-workspace-motion.css`. The upstream rule centres the entire flex row immediately while the sidebar still occupies width; at 1440px its left edge jumps to128px and drifts to262px during collapse. The Gi override anchors that row and interpolates the chat margins without changing its flex basis. The toggle's left offset follows the same transition. Desktop editor layouts and narrow drawers retain supplied rules. Reduced motion disables the affected transitions. Supplied component/source CSS files remain unchanged.

The regression uses trusted pointer and keyboard activation with requestAnimationFrame samples, including rapid reversal, no layout overshoot, synchronised widths/positions and exact stored draft/media preservation. Chromium and WebKit passed at1024/1440/1920, then restored the original project viewports. Complete workspace/shell/Settings regression:204 passes. Functional:100 passes/five existing skips; Go tests,vet and hook checks pass. A timed-out partial run is excluded. This repairs a shared Piclaw defect deliberately and adds no frozen mapping or terminal UI.

Deployment remains gated on native auth CI. Logs for36063764465 are now available: Windows reports `process lock not exclusive: <nil>` in `TestAuthStateCrossProcessAndCrashRelease`; macOS reports `openat auth.lock: no such file or directory` in `TestAuthStateConcurrentEnrollmentHasOneWinner`. Live8090 remains unchanged while those failures are investigated separately.

Deployment gate resolved: `6bc5c08` separates exclusive lock creation from existing opens and fixes the test child that could exit through Go's deadlock detector. Native CI36071594429 passed all three auth platforms, main Test and five builds. Restarted8090 at PID2404942 with that commit (including motion fix `f33b466`). Twelve mutation-blocked live browser checks passed: six frame/reversal/reduced-motion probes and six responsive status/draft probes. Database integrity is ok;62sessions/51turns/146messages and normalized contents are unchanged except runtime lease renewal. Auth remains unenrolled. Fresh-chat/Return, composer/picker reference differences, slash/Quick Actions and full browser CI wiring remain open.

### Browser-owner proof prerequisite (2026-09-25)

`c09f788` adds persisted browser-owner purpose and session-scoped five-minute TOTP proof, with cookie-only status/reauth APIs and an in-transaction guard for future credential changes. Native/bearer and legacy sessions retain ordinary access but cannot acquire management authority by being copied into a cookie. Reauth changes neither token nor expiry and grants no proof to another browser. Older writers dropping new session fields fail closed for management. See `browser-auth-proof.md`.

Auth race x10, web/auth race x3,24 browser cases,101 functional/five existing skips, Go/vet/hook and five cross-builds passed; independent security review found no blockers. CI36075923177 passed all three native auth jobs, main Test and five builds. Deployed8090 PID2457971; six mutation-blocked live checks passed, proof GET without owner cookie returns401, auth remains unenrolled, DB integrity/counts/normalized dump unchanged except runtime lease renewals. WebAuthn ceremonies, RP/origin configuration, multi-key persistence and Settings are still outstanding; no additive passkey credit or terminal UI changes.

### Native WebAuthn backend (2026-09-25)

`901d746` adds opt-in pure-Go WebAuthn verification, explicit RP/origins, per-credential storage, durable bounded one-use challenges and native registration/login/reauth/list/rename/remove routes. Recent browser proof, current credentials/policy and atomic last-factor checks fence commits. Registration/assertion tests use actual Chromium credential ceremonies with distinct virtual key material; malformed proof is consumed, counter clone warnings rejected, and last-used metadata is credential-scoped. See `passkeys.md` for API/storage limits. No Settings/login controls or policy editor yet; all26complete additive scenarios remain unmapped, and physical/synced devices remain unverified.

12Chromium API cases at3sizes cover two-key restart/independent login, passkey-only further enrolment, rename/remove/rejected removed-key/last-factor, session/revoke/replay/expiry/origin and signed wrongRP checks, cancellation, duplicate-ID and uncertain finish.24TOTP browser regressions,102functional/five existing skips,auth race×10/web×3,Go/vet/hook/5crossbuilds pass. Focused security review found no blockers. CI36079153337 now requires the passkey browser job as well as native three-OS auth/main Test/five builds, all green. Deployed8090 PID2507187 with RP/origins absent; six mutation-blocked live checks passed, passkey list403/auth unenrolled,DBintegrity/counts62/51/146/normalized hash unchanged except lease. No terminal changes or new idle rows.

### Multi-passkey Settings/login (2026-09-25)

`446baad` wires native ceremonies into a lazy Authentication pane and passkey-aware login gate. Server capability flags gate controls; successful sign-in confirms cookie authority. Settings supports list/add/rename/remove/confirm, TOTP or passkey recent proof, explicit retry/refresh, cancellation and local-orphan/uncertain-response explanations. Native prompt blur is not cancellation; closing/changing panes aborts and fences late completion, Escape first cancels local work/confirmation, and focus returns after cancellation. Full credential identifiers distinguish rows. No supplied components or terminal rows changed.

21Chromium virtual-authenticator API/UI cases,30auth browser cases,354Settings/QuickActions,103functional/five skips,104helpers1334assertions,raceauth×10/web×3,Go/vet/hook/5crossbuild passed. Actual UI tests add two keys, restart/login each, retain draft/media, rename/remove/cancel, reauthenticate via passkey and add/login without TOTP; faults cover stale proof/reads/writes/uncertainfinish/nativecancel/unmount. Six-project unavailable-state tests include WebKit, but no WebKit ceremony or physical/synced-key proof. Full26scenario mapping,Visual skin,policy editor and initial-owner bootstrap remain open. CI36083298328 passed allnative/browser/build jobs;deployed8090PID2591087 with RPabsent.6mutation-blocked live Authentication checks passed;unenrolled status,DBintegrity/counts62/51/146/normalized dump unchanged except lease.

### Sign-in policy and per-scenario review (2026-09-25)

`71f43c3` adds owner-only policy GET and fresh-proof CAS POST, explicit Settings choice/confirmation/cancel and failure refresh. Target either/totp-only/passkey-only must retain a verified accepted factor in the current write snapshot. CAS, current-policy proof and factor counts share auth.lock with key removal; neither credentials nor sessions are changed by policy selection. TOTP-only still permits reauthentication/policy changes and shows configured-RP keys read-only. New random revision detects stale browser views. No extra TUI rows.

27passkeyChromium cases,36auth6projects,216Settings,104functional/five skips,104helpers1334assertions,authrace×10/web×3,Go/vet/hook/5crossbuild pass;final security review no blockers. UI covers transitions/current-proof change,staleCAS,failed/lost response and policy change while removal confirmation is open;native multi-manager race cannot lock out. `passkey-scenario-review.md` audits all26 cases:004/014/022/025 are automated candidates,others partial/manual/unsupported. No formal credit yet. CI36086022919 allnative/browser/buildjobs green;deployed8090PID2640270 RPabsent,6mutation-blocked live checks pass/policyGET401/authunenrolled;DBintegrity/counts62/51/146/normalized hash unchanged except lease. Initial owner bootstrap,fullcriterion mapping,Visual skin/physical devices/nativeprompt and productionRP selection remain open.
