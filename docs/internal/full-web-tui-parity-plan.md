...
The session mutation slice (`015`) is verified; see [ADR-0009](../adr/0009-session-picker-mutations.md). The terminal draft/event isolation and bounded selector slice is also verified. Next: durable browser drafts and queue/model/reconnect ownership, plus pending terminal media refs, mutation submenus and durable queue acceptance. Remaining lifecycle cases still need their own evidence; see [web-session-parity.md](web-session-parity.md).
