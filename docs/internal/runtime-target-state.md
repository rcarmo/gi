...
- shell terminal `session_state` hook payloads now carry `turn_id` just like the paired `session_idle` runtime topics, and the generic terminal `finishTurnWithPayload(...)` path now does the same; setup-time `session_running` / `session_state` payloads now also share `turn_id`, `active_turn_id`, `turn_status`, and `turn_phase`, and terminal/queued-cancel session lifecycle payloads now carry `turn_phase` consistently too, so hook consumers and topic consumers no longer see different turn correlation data for the same lifecycle edge
