...
- the guarded web runtime surface now exposes that narrow queue path via `/api/runtime/inbound-work` (enqueue/list with optional `eligible=true|false` filtering and queue counts), `/api/runtime/inbound-work/drain` (bounded drain), and `/api/runtime/inbound-work/requeue` (manual recovery for `failed`/`retry` rows), so the inbound queue is reachable through a real runtime caller instead of only tests/internal code
...
- the first queue slice is store/schema + claim/status APIs, plus a minimal engine-side enqueue/claim/process/drain path so later IPC/CLI/system ingress can converge on one durable queue without forcing the wider bus design decision up front
