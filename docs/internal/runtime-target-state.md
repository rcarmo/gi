...
- same-session queue coordination now keeps persisted session state aligned with actual queue/claim truth: `CancelTurn(...)` honors the caller-supplied session boundary instead of trusting only the turn id, queued-turn cancellation derives post-cancel session state from both queued depth and any still-active turn claim instead of incorrectly downgrading a still-running session to `idle`/`queued`, `SubmitPrompt(...)` marks sessions `queued` (with cleared `active_turn_id`) when work remains queued instead of leaving stale `idle` state with only `queue_count` incremented and now also preserves the existing session model when the current submit omits one, and staged steering continuations now sync `queue_count` plus queued session state before later launch without blanking an existing session model when the queued steering payload omits one, so continuation-created queued turns do not leave stale idle state or regress model state behind
