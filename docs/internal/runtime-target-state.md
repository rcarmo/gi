...
- lower-level helper fallbacks are now explicit instead of secretly allocating raw `context.Background()` internally: engine-owned call sites pass a deliberate fallback context when canonical identity or failure-marker durability must survive caller cancellation, while bare helper calls without an explicit fallback now degrade locally instead of escaping engine lifecycle ownership
