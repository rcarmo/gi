...
- this means the current runtime now supports both "one session, many channel identities" (via bindings) and "temporary continuation from another channel" (via explicit continuation + bound reuse), while deliberately rejecting broader automatic cross-channel policy for now: unrelated channel/account/chat identities are not linked unless an explicit continuation or existing binding says they are the same logical session, and outbound fan-out to all bound channels is not part of the current policy
...
- main-session semantics still only cover one preferred session per `(agent, channel, account)` tuple; richer policy remains explicit-continuation/binding driven rather than automatic cross-channel merging
- automatic cross-channel continuation and outbound fan-out are deliberately out of scope for the current runtime contract; any future change must introduce an explicit policy layer above `session_channel_bindings`
...
- setup/finalization-adjacent helper reads that still influence coordination outcomes (for example turn agent/model resolution and durable route-decision recording) now also fall back to the engine-owned background context when the caller context is already canceled, so canonical session identity and routing audit/state do not silently degrade back to default behavior under cancellation pressure
- lower-level helper fallbacks are now explicit instead of secretly allocating raw `context.Background()` internally: engine-owned call sites pass a deliberate fallback context when canonical identity or failure-marker durability must survive caller cancellation, while bare helper calls without an explicit fallback now degrade locally instead of escaping engine lifecycle ownership; the same cleanup now extends to the web inbound-dispatcher lease-release path, which now uses a detached server lifecycle context instead of a raw background context during shutdown, the dispatcher entrypoint itself now defensively normalizes `nil` caller context to a real background context before deriving detached shutdown state and only starts once per server instance so duplicate startup calls cannot spawn competing same-owner dispatcher loops, and the `cmd/gi` web runtime now starts the dispatcher plus HTTP/HTTPS servers under a real process lifecycle context (`signal.NotifyContext`) so shutdown can release runtime leases and close listeners cleanly instead of pinning those paths to immortal background contexts; the auxiliary ACME HTTP-01 listener now also runs through the same shutdown helper instead of bypassing graceful process teardown in a fire-and-forget `ListenAndServe` goroutine
- subturn/effective-tool metadata parsing is now also more centralized: inherited `effective_tools` lookup, per-turn allowance checks, and tool-definition filtering share the same helper path while filtered tool definitions continue to preserve registry ordering instead of metadata ordering, and supported metadata shapes (`[]string`, `[]any`, comma-separated strings) now normalize through one trim/filter/dedupe helper instead of repeating that logic across cases
- lower-level routing/direct parsing now also shares a tiny normalization helper for repeated lowercase+trim behavior (`normalizeAgentID`, direct kind/source normalization, and scoped-value parsing), with focused regression coverage to keep the parse semantics stable while reducing duplicate string-munging code; the repeated same-session normalization paths now also share helpers so both the "running session from active turn" cases and the complementary queued/idle no-active-turn cases resolve queue count plus model/active-turn shape through one place instead of drifting independently across steering, continue, queued-turn handoff, staged continuation submit, and cleanup-time re-normalization; those normalization helpers now also return real errors so `ContinueSession`, staged steering continuation, queued-turn handoff, and launch-conflict steering fallback stop reporting success when queue-count/session-state reconciliation fails instead of silently swallowing the missing-session/write failure path, post-run cleanup now aborts the rest of its next-work sequencing when queue-count reconciliation has already failed instead of continuing into hooks / queued-turn launch / steering continuation on a broken session, that same cleanup path now also aborts immediately when releasing the active-turn claim itself fails instead of continuing into next-work sequencing while the session may still appear owned, and both early-stop cleanup branches now append durable `turn.cleanup_handoff_failed` audit events so the aborted handoff is recorded instead of only appearing in logs; successful post-run cleanup handoff into the next queued turn now also appends a durable `turn.cleanup_handoff` event on the finishing turn, and cleanup now records `turn.cleanup_handoff_failed` with a dedicated `continue_queued_steering` stage when the no-queued/no-active continuation path fails after claim release. Successful queued steering continuation handoff now also appends a durable `steering.continued` event so continuation completion is recorded explicitly instead of only being inferred from staged/submitted breadcrumbs plus live broadcast, queued cancel now appends a durable `turn.cleanup_handoff` marker whether it hands off to the next queued turn, back to an already-running active turn, or down to an idle session instead of leaving those sequencing edges visible only through live setup/session-state notices, and launch-claim conflict steering fallback now appends a durable `turn.cleanup_handoff` marker onto whichever turn absorbed or preserved the prompt — the active turn when steering succeeds, or the transient queued turn when steering is rejected and the queued fallback is intentionally preserved — so that claim-conflict resolution is explicit in audit rather than only visible as a generic steering enqueue/rejection plus live running-state consequences. Phase 2 session-identity cut-over is now also using `session_identities` as the canonical runtime source for allocation matching and routed inbound-context reconstruction: alias-based allocation reuse no longer trusts `sessions.scope_json` to prove scope equivalence, routed inbound context no longer rehydrates channel/account/chat/topic/space from stale session JSON when a persisted identity exists, the turn helpers that derive agent/channel/account for routed work now prefer store-backed identity and only fall back to legacy scope/defaults when there is no resolvable session identity at all, turn-time source-agent derivation now resolves directly from persisted identity by session id instead of reloading full session rows just to rediscover `agent_id`, sibling child-session reuse now resolves through store-backed parent/linkage lookup rather than trusting the caller's loaded source row to carry the parent session relationship correctly, the routed session planner now carries source session ids rather than source session rows for same-agent short-circuiting plus reuse/fork decisions so route/session selection depends on store-backed identity/linkage instead of row-shaped identity snapshots, and the store/runtime now has ID-first resolution helpers for main-session, key/alias, opaque-key, alias, canonical-signature, channel-binding, child/sibling-child lookup, and plain session existence checks so direct ingress, TUI bootstrap, allocation matching, routed child-session reuse, and resolve-or-create session/main-session flows can stay in canonical session-id space until a path actually needs full session payload/state fields.  Stale-claim recovery now surfaces per-claim recovery failures (including recovery hold-marker persistence failure) back to the caller instead of only logging and continuing as though the scan completed cleanly; failed per-claim recovery attempts now also append durable `turn.recovery_failed` audit events keyed by the recovery disposition that was being attempted, so interrupted-turn scans leave behind more than transient logs when a session could not be recovered, those failed recovery attempts now publish matching live `runtime.turn` / `runtime.session` failure checkpoints so the live runtime surfaces have the same visibility as the durable audit trail, mixed-outcome recovery scans now also emit a session-level `session_state` summary with `reason=recovery_scan_failed` plus recovered/failed claim counts so partial scan failure is visible per failed session instead of only as one returned error across the whole scan, and the failed turns inside that session now also get durable `turn.recovery_scan_failed` summary rows carrying the same aggregate counts so the mixed outcome can be reconstructed from audit alone; recovery paths that successfully release a stale claim but then fail queued-work restart now emit an explicit `session_state` with `reason=recovery_restart_failed` and append a durable `turn.recovery_restart_failed` event onto the queued turn that could not be relaunched, so that post-recovery handoff failure is visible in both live session state and turn audit instead of only through the returned error. Script-topic subscription cleanup now also eagerly drops closed host-subscription handles on read so dead topic subscriptions do not accumulate silently inside `ScriptTool`, while topic unsubscribe becomes idempotent so scripts can still explicitly unsubscribe after a drained/closed read path without turning cleanup into a spurious tool error, raw/websocket close operations now follow the same idempotent cleanup contract so repeated socket teardown does not fail just because a prior cleanup path already removed the handle, script-facing topic/raw-socket/websocket handles are now also session-owned instead of only globally keyed so one session cannot read, write, or close another session's guessed handle IDs, script-facing connectivity route registration/listing/unregister now also enforce the current session boundary at the web bridge so session-scoped script APIs no longer create, expose, or remove another session's process-global routes by default, and script-facing topic publish/subscribe now enforce that same current-session boundary at the web bridge so scripts cannot observe or emit on behalf of another session by overriding `session_id`; the web bridge now also normalizes route filters and emitted connectivity payloads via detached copies instead of mutating caller-owned maps in place, web metrics sampling now also avoids holding the collector mutex across `/proc` reads and `runtime.ReadMemStats`, and the returned time-series snapshots are detached copies instead of aliases into the collector's live backing slices, pending auth enrollments now proactively prune expired entries both when verification notices expiry and when a fresh enrollment starts, instead of leaving stale in-memory enrollment entries behind for unrelated users, auth enrollment start now surfaces persisted auth-state load errors (for example corrupt on-disk JSON) instead of silently treating them as "not enrolled" and proceeding over bad state, auth enrollment start HTTP handling now classifies those surfaced state-load failures as server errors instead of misreporting them as bad client requests, auth enrollment verify HTTP handling now classifies persistence/state failures as server errors instead of misreporting them as bad client requests, auth TOTP verification HTTP handling now classifies persisted-state load failures as server errors too instead of collapsing them into credential failures, and web auth gating / route-level TOTP bearer checks now use explicit error-returning validation helpers so corrupt persisted auth state yields server errors on protected endpoints instead of silently degrading into ordinary unauthorized responses; connectivity route HTTP auth handling now likewise distinguishes expected auth failures (401) from internal auth/config/state failures (500) instead of collapsing every authorization error into unauthorized, connectivity SSE streaming now stops on event-marshal failure instead of silently skipping the bad event and continuing the stream, `/api/tools/execute` now preserves a script input's own `session_id` when the outer request field is blank instead of silently clobbering it with an empty value, store-side active-turn heartbeat/touch now returns a missing-row error instead of silently succeeding when the claim token no longer matches any active turn row, the running-turn heartbeat now treats that lost-claim condition as fatal by cancelling the in-flight turn instead of letting execution continue under an unowned session, store-side session-state patching now also returns a missing-row error instead of silently succeeding when launch/recovery/continue normalization tries to update a session row that is already gone, the core turn lifecycle writers (`update status/phase`, `mark claimed`, `reset claim`, `mark finished`) now also fail closed on missing turns instead of silently succeeding on zero-row updates, and launch-time rollback now surfaces cleanup-time coordination failures as part of the returned error instead of only logging them while reporting the original setup failure; auth state load/save no longer hold the manager mutex across filesystem I/O and JSON encode/decode work because those paths only touch immutable config plus local state; frontend log ingestion now also reports JSON-marshal failures for `detail` via a fallback structured string instead of silently logging an empty payload when the browser sends an unserializable detail shape; connectivity basic-auth secret resolution now also honors the generic `keychain` field the same way bearer/header/query/HMAC auth already did, so unsupported keychain-backed basic-auth configs fail with the intended placeholder error instead of a misleading "password is required" path, and `options.allow_unauthenticated_external=true` on connectivity routes now truly bypasses external auth enforcement as documented even when an auth block is present, instead of only affecting routes with no configured auth; the web-layer TOTP special-case now honors that same bypass option too instead of reintroducing the old auth requirement only for `auth.type="totp"` routes, script event hooks are now tracked per session rather than in one global list so `clearEventHooks()` and event emission do not let one session wipe or observe another session's registrations, and when host/engine-backed event hooks are registered their unregister callbacks are now retained per session too so `clearEventHooks()` actually tears down runtime-registered hooks instead of only clearing the local fallback registry, store-side turn deletion now treats missing turns as a no-op so best-effort rollback/delete paths do not turn already-cleaned state into new orchestration errors, connectivity route unregister now follows the same cleanup contract so repeated route teardown does not promote already-removed session routes into fresh errors, connectivity route lifetime defaults now match actual behavior (`process`) instead of advertising session-scoped cleanup that the current registry does not yet implement, and connectivity event-bus unsubscribe now has explicit regression coverage for idempotent and concurrent cleanup so repeated teardown cannot reintroduce channel-close panics unnoticed
- same-session queue coordination now keeps persisted session state aligned with actual queue/claim truth: `CancelTurn(...)` honors the caller-supplied session boundary instead of trusting only the turn id, queued-turn cancellation derives post-cancel session state from both queued depth and any still-active turn claim instead of incorrectly downgrading a still-running session to `idle`/`queued`, `SubmitPrompt(...)` marks sessions `queued` (with cleared `active_turn_id`) when work remains queued instead of leaving stale `idle` state with only `queue_count` incremented and now also preserves the existing session model when the current submit omits one, its same-session active-claim probe now uses the same caller-or-engine-owned coordination context as the downstream steering path so already-canceled callers still steer into the claimed turn instead of missing the active session claim before enqueue, and its plain queued-submit path now also uses that coordination-aware context for queue-depth lookup plus durable turn creation so already-canceled callers can still enqueue queued work before the later durable bookkeeping runs, the public direct inbound-work enqueue path now uses the same fallback-aware coordination context for durable queue insertion so already-canceled callers can still stage IPC/system/direct work into `inbound_work_queue`, same-session prompt steering now also re-normalizes persisted session `status` / `active_turn_id` / model plus `queue_count` when a new prompt is serialized into steering against an already-active turn and now uses the same caller-or-engine-owned durable coordination context rule so the steering enqueue path itself survives already-canceled callers, staged steering continuations sync `queue_count` plus queued session state before later launch without blanking an existing session model when the queued steering payload omits one, their store-side staging path now returns the durable queued turn record directly instead of re-reading it through a raw `context.Background()` post-commit path, queued-turn launch now re-syncs `queue_count` immediately after a queued turn is claimed/running and pre-populates session model from the launched turn before setup begins so follow-on handoff paths do not leave stale queued counts or stale model state behind, and its early-return handoff branches now also normalize the already-claimed running turn plus queue count/model instead of silently returning while queued session state lingers, if that pre-run launch path fails after the turn was marked claimed, the runtime now resets the turn’s claim bookkeeping, restores queued turn/session state, and re-syncs `queue_count` before releasing the active claim so partial local launches do not strand claimed metadata behind, continuation handoff now re-normalizes session state back to `running` when another worker claims the staged continuation turn before local launch and now also restores the claimed continuation turn's model onto the session while re-syncing `queue_count`, queued-turn handoff inside `ContinueSession(...)` now also normalizes running session state when another worker claims the queued turn between the idle check and local launch attempt and re-syncs `queue_count` when that external claimant has already moved the turn out of the queue, active `ContinueSession(...)` calls now also normalize persisted session `status` / `active_turn_id` / model back to the already-claimed running turn, while also re-syncing stale `queue_count` instead of returning early while queued state lingers, idle `ContinueSession(...)` calls now normalize stale queued session state back to `idle` when no queued turn, active claim, or staged continuation work actually remains, launch-conflict steering fallback now re-syncs `queue_count` plus running session state after deleting the transient queued turn and now also restores the active-turn model when the incoming prompt omitted one, so claim-conflict conversion does not leave stale queued or model state behind, and final cleanup now re-normalizes session `status` / `active_turn_id` from active-claim plus queue truth after release/handoff attempts while also restoring the externally claimed turn model, so externally claimed follow-on work does not leave a stale idle session or stale model behind
- hold-resolution runtime turn topics now publish the post-update phase rather than the stale pre-update turn phase, so `turn_failure_held` / `turn_failure_resolved` stay semantically aligned with the store’s updated turn state when a failure is held or resolved; their held-payload fields (`hold_state`, synthesized default `summary`) are now normalized before publication too, so the live topic payload matches the persisted `turn_failures` row instead of echoing raw caller input
- shell terminal `session_state` hook payloads now carry `turn_id` just like the paired `session_idle` runtime topics, and the generic terminal `finishTurnWithPayload(...)` path now does the same; setup-time `session_running` / `session_state` payloads now also share `turn_id`, `active_turn_id`, `turn_status`, and `turn_phase`, terminal/queued-cancel session lifecycle payloads now carry `turn_phase` consistently, and idle session hook payloads now explicitly clear `active_turn_id` just like the paired `session_idle` runtime topics, so hook consumers and topic consumers no longer see different turn correlation data for the same lifecycle edge
...
- `ContinueSession(...)` now runs its idle no-active check, queued-turn launch attempt, and steering-continuation staging/launch under that same shared session coordination lock, so manual continue behaves like one atomic same-session ownership decision instead of a sequence of loosely related checks; the continuation path now also falls back to engine-owned durable context when the caller context is already canceled so queued continuation staging/launch does not disappear with the initiating request
...
- stale active-turn recovery on engine startup and before same-session submission; recovery-side stale-claim repair now also falls back to engine-owned durable context when the caller context is already canceled so `turn.recovered`, claim release, session-state normalization, and queued restart work are not dropped mid-repair
- advisory `turn_failures` rows for durable failure/recovery postmortems; failure-marker helpers now also survive already-canceled caller contexts so `turn_failures` rows and `turn.failure_marked` audit checkpoints are not lost in detached terminal cleanup
...
- hold application and hold resolution now also publish explicit canonical `runtime.turn` checkpoints (`turn_failure_held`, `turn_failure_resolved`) so review/skip/retry decisions are not visible only through stored turn events, and the hold/skip/retry control paths now fall back to engine-owned durable context when the caller context is already canceled so their failure-row and audit mutations still land
...
- queued turn creation now also publishes an explicit canonical `runtime.turn` checkpoint (`turn_submitted`) for both ordinary queued submits and queued steering-continuation staging, so queue entry is not visible only through stored `turn.submitted` audit rows; the queued-launch helper beneath those paths now also falls back to engine-owned durable context when the caller context is already canceled so claim/setup/session-running writes still reach the runtime
...
- sub-turn lifecycle publication on topic bus (`subturn_created`, `subturn_status`, `subturn_result_ready`, `subturn_result_delivered`, `subturn_orphaned`, `subturn_cancel_requested` → `turn.subturn`), and parent-session sync/orphan delivery notices now fall back to engine-owned durable context when the caller context is already canceled so cross-session result/orphan notices are not dropped during detached terminal cleanup
...
- direct-origin turns now stamp normalized ingress audit metadata onto the same persisted audit surfaces used by normal chat-origin turns: turn metadata, persisted user-message payloads, and `turn.started` event payloads (`ingress_kind`, `ingress_source_kind`, `ingress_source_id`, `ingress_role`, `ingress_label`, `ingress_channel`, `ingress_account`, `ingress_session_key` when provided), and direct prompt/peer-message/continue handling now falls back to engine-owned durable context when the caller context is already canceled so explicit session-key resolution and downstream submit/continue actions still reach the normal runtime path
...
- core runtime turn/session lifecycle checkpoints now also publish onto the canonical topic bus under `runtime.turn` and `runtime.session`; this now includes generic `turn_state` / `session_state` notices emitted from the shared state-hook path plus explicit setup, queue-submission, hold-resolution, and terminal/idle checkpoints, with the explicit checkpoint path kept non-duplicated, completed edge paths normalized onto `turn_completed` / `turn_completed`-reasoned idle semantics, completed exits now carrying explicit completion metadata such as `iterations` / `completion_kind` across both turn and session runtime notices, interrupted-turn recovery publishing explicit `turn_recovered` checkpoints plus generic recovery `turn_state` / `session_state` notices while also restarting queued follow-up work automatically when stale-claim recovery leaves runnable queued work behind, active cancellation publishing explicit `turn_cancelling` checkpoints plus richer session-state metadata, queued cancellation publishing explicit terminal turn/session checkpoints when it leaves the session idle while also continuing later queued work when queued follow-up remains, queued turn creation publishing explicit `turn_submitted` checkpoints, held-failure review/skip/retry resolution publishing explicit `turn_failure_held` / `turn_failure_resolved` checkpoints, setup failure/cancel terminal finalization now preserving resolved model/agent identity before publishing terminal runtime/session notices, shell terminal audit rows now normalized across completed/cancelled/failed outcomes, shell tool audit rows now use the same high-level payload conventions as the normal tool path (`output_length`, `failure_kind`), and cancel bookkeeping now prefers durable context when the request context is already canceled, while still stopping short of a full mirror of every stored turn event row
...
- persisted routing/allocation decision notices now also publish onto the canonical topic bus under `runtime.routing`; this currently covers route-decision plus incoming-route notices emitted from the same normalized decision object that is stored in `routing_events`, rather than every pre-persistence route-resolution branch; the routing-side source/fork system notices and routed-peer downstream submit path now also fall back to engine-owned durable context when the caller context is already canceled so those side-channel notices do not outlive the routed action or vanish after the route/session action already committed
