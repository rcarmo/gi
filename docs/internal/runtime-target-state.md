...
- lower-level routing/direct parsing now also shares a tiny normalization helper for repeated lowercase+trim behavior (`normalizeAgentID`, direct kind/source normalization, and scoped-value parsing), with focused regression coverage to keep the parse semantics stable while reducing duplicate string-munging code
