...
- the repeated engine-side "coordination context" selection rule (use caller context when still live, otherwise fall back to engine-owned background context) is now centralized in a shared helper and reused across direct ingress, inbound-work post-claim bookkeeping, routing preparation/decision helpers, hold-resolution, steering, launch/finalize helpers, and queued-turn recovery to reduce drift between equivalent cancellation-sensitive code paths
