...
- core runtime turn/session lifecycle checkpoints now also publish onto the canonical topic bus under `runtime.turn` and `runtime.session`; this currently covers setup start plus terminal/idle transitions, not a full mirror of every stored turn event row
