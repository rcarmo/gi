...
- lower-level routing/direct parsing now also shares a tiny normalization helper for repeated lowercase+trim behavior (`normalizeAgentID`, direct kind/source normalization, and scoped-value parsing), with focused regression coverage to keep the parse semantics stable while reducing duplicate string-munging code; the repeated "running session from active turn" normalization path now also shares one helper so steering fallback, steering submit, continue handoff, queued-turn handoff, and cleanup-time re-normalization all resolve queue count plus active-turn model through the same code instead of drifting independently
