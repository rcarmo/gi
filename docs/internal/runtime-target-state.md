...
- `CancelTurn(...)` now honors the caller-supplied session boundary instead of trusting only the turn id: mismatched session/turn combinations are rejected and leave queued turns untouched rather than allowing accidental cross-session cancellation by turn id alone; cancel-related runtime topics and session/turn state hooks now also resolve and propagate the turn's agent/model context instead of emitting empty model/agent fields during queued/active cancel transitions, and queued-turn cancellation now derives post-cancel session state from both queued depth and any still-active turn claim instead of incorrectly downgrading a still-running session to `idle`/`queued`
