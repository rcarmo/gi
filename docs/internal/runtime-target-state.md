...
- subturn/effective-tool metadata parsing is now also more centralized: inherited `effective_tools` lookup, per-turn allowance checks, and tool-definition filtering share the same helper path while filtered tool definitions continue to preserve registry ordering instead of metadata ordering
