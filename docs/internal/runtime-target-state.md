...
- the TUI now also has a first topic-native consumption slice for the active session, subscribing to canonical topic families with session scoping and using `runtime.tool` / `runtime.hook` / `runtime.turn` / `runtime.session` plus `session.compaction` / `session.routing` / `session.steering` / `turn.subturn` notices directly for status and transcript updates; for those adopted families, overlapping legacy session-broadcast handling now serves as fallback compatibility rather than the primary rendering source
