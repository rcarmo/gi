...
- `CancelTurn(...)` now honors the caller-supplied session boundary instead of trusting only the turn id: mismatched session/turn combinations are rejected and leave queued turns untouched rather than allowing accidental cross-session cancellation by turn id alone
