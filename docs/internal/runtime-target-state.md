...
- per-handler hook invocation lifecycle/decisions now also publish onto the canonical topic bus under `runtime.hook`, carrying hook name/source/action/error/duration metadata alongside the durable `hook_invocations` audit rows
- core runtime turn/session lifecycle checkpoints now also publish onto the canonical topic bus under `runtime.turn` and `runtime.session`; this now includes generic `turn_state` / `session_state` notices emitted from the shared state-hook path plus explicit setup and terminal/idle checkpoints, while still stopping short of a full mirror of every stored turn event row
