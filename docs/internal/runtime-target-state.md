...
- hold application and hold resolution now also publish explicit canonical `runtime.turn` checkpoints (`turn_failure_held`, `turn_failure_resolved`) so review/skip/retry decisions are not visible only through stored turn events
...
- queued turn creation now also publishes an explicit canonical `runtime.turn` checkpoint (`turn_submitted`) for both ordinary queued submits and queued steering-continuation staging, so queue entry is not visible only through stored `turn.submitted` audit rows
...
- direct-origin turns now stamp normalized ingress audit metadata onto the same persisted audit surfaces used by normal chat-origin turns: turn metadata, persisted user-message payloads, and `turn.started` event payloads (`ingress_kind`, `ingress_source_kind`, `ingress_source_id`, `ingress_role`, `ingress_label`, `ingress_session_key` when provided)
