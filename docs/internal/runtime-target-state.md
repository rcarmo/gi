...
- the TUI now also has a first topic-native consumption slice for the active session, subscribing to canonical `runtime.*` topics and using `runtime.tool` / `runtime.hook` notices directly for status and transcript updates alongside the older session-broadcast path
