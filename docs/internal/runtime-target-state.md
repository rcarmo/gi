...
- shell terminal `session_state` hook payloads now carry `turn_id` just like the paired `session_idle` runtime topics, and the generic terminal `finishTurnWithPayload(...)` path now does the same, so hook consumers and topic consumers no longer see different turn correlation data for the same idle transition
