...
| 014 | Lost successful finish, UI uncertain-result explanation, explicit list refresh adds one row, no immediate second finish. | Partial: the frozen journey returns to Passkeys; the test clicks Refresh without leaving/re-entering the pane. |
...
| 020 | Native current-RP/policy/factor removal matrix; selected UI last-key refusal. | Partial: full seven-example Settings matrix absent. Generic refusal does not supply the required distinct session-only, old-RP, TOTP-fallback and unverified-TOTP explanations. |
