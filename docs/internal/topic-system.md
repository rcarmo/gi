...
A first topic-native slice is now live: the TUI subscribes to canonical topic families for the active session and uses `turn.status`, `turn.response`, `runtime.tool`, `runtime.hook`, `runtime.turn`, `runtime.session`, `session.compaction`, `session.routing`, `session.steering`, and `turn.subturn` notices for status and transcript updates. Where those topic-native paths are active, the overlapping legacy broadcast handlers are now treated as fallback compatibility paths rather than the primary source of truth.
