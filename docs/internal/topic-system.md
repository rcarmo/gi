...
- hook invocation lifecycle publication plus higher-level hook decision notices under `runtime.hook` (including tool-call/approval/result abort, deny, modify, and direct-respond outcomes); hook invocation audit persistence now also uses the engine background context instead of raw `context.Background()` so it survives request cancellation without becoming process-lifetime work
