...
- broader TUI/topic-native coverage beyond the first adopted runtime/session families and status paths
...
A first topic-native slice is now live: the TUI subscribes to canonical topic families for the active session and uses `runtime.tool`, `runtime.hook`, `runtime.turn`, `runtime.session`, `session.compaction`, and `session.routing` notices for status and transcript updates alongside the older broadcast channel.
