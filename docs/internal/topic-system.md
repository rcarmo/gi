...
A first topic-native slice is now live: the TUI subscribes to canonical topic families for the active session and uses `turn.status`, `turn.response`, `turn.draft`, `turn.thought`, `runtime.tool`, `runtime.hook`, `runtime.turn`, `runtime.session`, `runtime.routing`, `runtime.inbound_work`, `session.compaction`, `session.steering`, and `turn.subturn` notices for status and transcript updates. Its `runtime.hook` rendering now covers invocation errors/timeouts plus abort/deny/modify/respond decisions, its `runtime.inbound_work` rendering covers queue/retry/failure/completion plus manual requeue/discard notices including retry attempt counts, `turn.draft` and `turn.status` running notices now mark the UI as actively running, and idle/completed/terminal turn/session notices (`turn.status` idle, `runtime.turn`, `runtime.session`) clear stale draft/running UI state. The cleanup path is now centralized in one TUI helper instead of duplicated across handlers, reducing drift between equivalent canonical lifecycle edges. Where those topic-native paths have a live active subscription, the overlapping legacy broadcast handlers are treated as fallback compatibility paths rather than the primary source of truth.
