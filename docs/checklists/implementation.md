...
- [ ] Implement workspace indexing from the source-backed lifecycle: explicit versioned migration, configured roots/scopes, incremental identity, atomic scoped cleanup, invalidation/background refresh, fenced ownership/recovery, native query/status/reindex and independent terminal acceptance; avoid restoring the global full-rebuild prototype as-is.
