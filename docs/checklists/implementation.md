...
- [ ] Reconcile display-idle with the native admission claim: terminal status can be complete while rapid prompt admission still routes as steering. Distinct history fixtures use queue intent; backend boundary requires separate tests/fix.
