...
- [-] P1 cross-tab draft recovery: disposable reproduction confirms stale whole-row writes erase a foreign pending capture, and stale unknown recovery can restore a draft after another tab's confirmed cleanup. Plan atomic per-session read/reconcile/write with exact-token settlement and two-page regression; preserve refs/media/six-cap and unknown warnings, never replay POST. Evidence `/workspace/tmp/gi-passkey-login-boundary/cross-tab-repro.ts` and `.log`; implementation pending.
