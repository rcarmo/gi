...
- [-] P1 auth cancellation implemented locally: deterministic held-request/held-response login-start regressions failed old Cancel UI6/6 then pass6/6; phase-owned cancellation only during credentials.get, start/finish stay single-flight busy, synchronous stale-button guard, late-start abort check. Repeat27/fullpasskeys195/auth120/functional129(11skips)/helpers8+1661assertions/core/vet/hooks pass; review no blocker. No auth-server/security/timeout changes or POST replay. CI/deploy pending; Settings cancellation and physical OS-sheet acceptance separate. [Boundary/evidence](../internal/passkey-login-cancellation.md).
