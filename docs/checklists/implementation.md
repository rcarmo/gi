...
- [-] Shared35 estimate/mapping slice locally verified: actual native tokens_before only with estimate provenance, active tooltip/aria separate from measured request, missing/malformed values omitted, no idle UI/backend arithmetic change. Canonical combined6project proof covers thinking,unknowns,real estimate,exact native compaction capability token; independent clause review accepted.36thinking/102compaction/18meter/6helpers62assertions/8report1963assertions/139functional(11skips)+core/vet pass. Source mapping32/42shared (Classic101/236 unchanged); focused1pass31not-run10unmapped. CI/deploy pending, no TUI/physical/pixel credit. [Contract/evidence](../internal/context-estimate-provenance.md).
...
- [-] Shared35 capability/estimate web evidence now passes in the combined canonical journey; mapping/estimate slice awaits exact-source CI/deployment above.
