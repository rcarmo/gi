...
- [ ] Prove `@shared-37` as one integrated native copy/delete flow across all six browser projects: stored Markdown/code clipboard bytes, success/failure glyph reset, real busy-run 409 then 200 for the captured ID, unaffected other session/message reference/draft. Focused 6/6, 80 functional, 39 helpers, Go/vet/build/hook pass; aggregated 51/236 Classic, 3/42 shared, 185/39 unmapped. Shipping pending. Terminal source-copy and selected-message deletion remain separate zero-idle-row designs.
