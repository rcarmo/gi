...
- [-] P1 cross-tab draft fencing implemented locally: atomic per-session revision CAS freezes stale writes/sends, retains local edits and refuses stale recovery before publishing; IndexedDBv2 fences old unconditional writers. No automatic merge/retry, full reload required after conflict/load failure; unknown-first/accepted-later remains unknown. Red2regressions then24helpers119assertions/32HTTPtwo-browser/139functional(11skips)/24controls/168drafts/18queue-return/core/vet/hooks pass; final review accepts bounded scope. Whole-product CI/deploy pending, livea1be540 unchanged. [Contract and limits](../internal/web-cross-tab-drafts.md); evidence `/workspace/tmp/gi-cross-tab`.
