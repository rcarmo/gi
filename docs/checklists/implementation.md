...
- [-] Shared35 thinking prerequisite implemented locally: catalogue-validated Settings choice, session/selection-revision CAS, model-bound immutable admission metadata into actual provider Reasoning options; active/queued choices retained, Default omitted, stale/foreign/unsupported rejects. Native race×3/30thinkingHTTP/42modelpanel/108Settings/139functional(11skips)/36controls/core/vet/hooks pass; review blockers fixed/final approval. Whole-product CI/deploy pending. Shared35 still unmapped (local-estimate clause unresolved); no estimates fabricated/TUIWIPchanges. [Contract](../internal/session-thinking.md).
