    ...
    return html`<div class="settings-dialog-backdrop" onClick=${e => { if (e.target === e.currentTarget) onClose(); }}>
        <div ref=${dialog} class="settings-dialog" role="dialog" aria-modal="true" aria-labelledby="gi-settings-title" onKeyDown=${e => e.stopPropagation()}>
            ...
            <div class="settings-dialog-body"><nav class="settings-nav" aria-label="Settings sections">
                ${['general', 'models', 'appearance'].map(id => html`<button class=${`settings-nav-item ${section === id ? 'active' : ''}`} aria-current=${section === id ? 'page' : undefined} onClick=${() => setSection(id)}>${{ general: 'General', models: 'Models', appearance: 'Appearance' }[id]}</button>`)}
            </nav><main class="settings-content">
                ${section === 'general' ? html`<${General} />` : section === 'appearance' ? html`<${Appearance} />` : html`<${Models} key=${chatJid} chatJid=${chatJid} onMutationStart=${onMutationStart} onMutationEnd=${onMutationEnd} onApplied=${onApplied} />`}
            </main></div>
        </div>
    </div>`;
}
