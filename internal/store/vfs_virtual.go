...
func (s *Store) listChatVFSChildren(ctx context.Context, dir string) ([]VFSListEntry, error) {
	...
	case "":
		return []VFSListEntry{
			{Name: "README.md", Path: "README.md", IsDir: false, ContentType: "text/markdown", Metadata: map[string]any{"virtual": true, "kind": storevfs.KindChatReadme}},
	...
	if len(parts) >= 2 && parts[0] == "sessions" {
		...
		if len(parts) == 2 {
			return []VFSListEntry{
				{Name: "session.md", Path: "sessions/" + sessionID + "/session.md", IsDir: false, ContentType: "text/markdown", Metadata: map[string]any{"virtual": true, "kind": storevfs.KindChatSession}},
				{Name: "messages", Path: "sessions/" + sessionID + "/messages", IsDir: true, Metadata: map[string]any{"virtual": true, "kind": storevfs.KindChatMessages}},
				{Name: "turns", Path: "sessions/" + sessionID + "/turns", IsDir: true, Metadata: map[string]any{"virtual": true, "kind": storevfs.KindChatTurns}},
			}, nil
		}
		if len(parts) == 3 && parts[2] == "messages" {
			...
			entries := []VFSListEntry{{Name: "index.md", Path: "sessions/" + sessionID + "/messages/index.md", IsDir: false, ContentType: "text/markdown", Metadata: map[string]any{"virtual": true, "kind": storevfs.KindChatMessagesIndex}}}
			for _, msg := range messages {
				entries = append(entries, VFSListEntry{Name: msg.ID + ".md", Path: "sessions/" + sessionID + "/messages/" + msg.ID + ".md", IsDir: false, ContentType: "text/markdown", Metadata: map[string]any{"virtual": true, "kind": storevfs.KindChatMessage, "role": msg.Role}, OriginalSize: len(msg.Content)})
			}
			return entries, nil
		}
		if len(parts) == 3 && parts[2] == "turns" {
			...
			entries := []VFSListEntry{{Name: "index.md", Path: "sessions/" + sessionID + "/turns/index.md", IsDir: false, ContentType: "text/markdown", Metadata: map[string]any{"virtual": true, "kind": storevfs.KindChatTurnsIndex}}}
			for _, tr := range turns {
				entries = append(entries, VFSListEntry{Name: tr.ID + ".md", Path: "sessions/" + sessionID + "/turns/" + tr.ID + ".md", IsDir: false, ContentType: "text/markdown", Metadata: map[string]any{"virtual": true, "kind": storevfs.KindChatTurn, "status": tr.Status}, OriginalSize: len(tr.Prompt)})
			}
			return entries, nil
		}
	}
...
`) + "\n"
	...
	if filePath == "sessions/index.md" {
		...
		return newVirtualVFSFile(storevfs.NamespaceChat, filePath, storevfs.KindChatSessionsIndex, body, "", ""), []byte(body), nil
	}
	...
	if len(parts) == 3 && parts[2] == "session.md" {
		...
		return newVirtualVFSFile(storevfs.NamespaceChat, filePath, storevfs.KindChatSession, string(raw), session.CreatedAt, session.UpdatedAt), raw, nil
	}
	...
	if len(parts) == 4 && parts[2] == "messages" && parts[3] == "index.md" {
		...
		body := strings.Join(lines, "\n") + "\n"
		return newVirtualVFSFile(storevfs.NamespaceChat, filePath, storevfs.KindChatMessagesIndex, body, session.CreatedAt, session.UpdatedAt), []byte(body), nil
	}
	...
	if len(parts) == 4 && parts[2] == "messages" && strings.HasSuffix(parts[3], ".md") {
		...
		body := msg.Content + "\n"
		raw := storevfs.RenderFrontmatterMarkdown(meta, body)
		return newVirtualVFSFile(storevfs.NamespaceChat, filePath, storevfs.KindChatMessage, string(raw), msg.CreatedAt, msg.CreatedAt), raw, nil
	}
	...
	if len(parts) == 4 && parts[2] == "turns" && parts[3] == "index.md" {
		...
		body := strings.Join(lines, "\n") + "\n"
		return newVirtualVFSFile(storevfs.NamespaceChat, filePath, storevfs.KindChatTurnsIndex, body, session.CreatedAt, session.UpdatedAt), []byte(body), nil
	}
	...
	if len(parts) == 4 && parts[2] == "turns" && strings.HasSuffix(parts[3], ".md") {
		...
		raw := storevfs.RenderFrontmatterMarkdown(meta, body)
		return newVirtualVFSFile(storevfs.NamespaceChat, filePath, storevfs.KindChatTurn, string(raw), tr.CreatedAt, tr.UpdatedAt), raw, nil
	}
