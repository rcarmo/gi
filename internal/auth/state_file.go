...
func readState(root *os.Root) (State, []byte, error) {
	...
	for _, key := range []string{"username", "totp_secret", "totp_enabled", "created_at", "updated_at", "sessions", "webauthn_user_id", "passkeys", "webauthn_ceremonies", "login_policy", "policy_revision"} {
		delete(object, key)
	}
	state.extra = object
