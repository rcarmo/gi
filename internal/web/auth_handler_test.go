...
func TestProtectedEndpointReturnsInternalServerErrorForCorruptAuthState(t *testing.T) {
	root := t.TempDir()
	...
	if res.Code != http.StatusInternalServerError {
		t.Fatalf("expected 500 for corrupt auth state on protected endpoint, got %d body=%s", res.Code, res.Body.String())
	}
}
