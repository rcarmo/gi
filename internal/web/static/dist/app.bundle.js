      ...
      onClose();
        ...
        <div ref=${dialog} class="settings-dialog" role="dialog" aria-modal="true" aria-labelledby="gi-settings-title" onKeyDown=${(e) => e.stopPropagation()}>
            ...
            <div class="settings-dialog-body"><nav class="settings-nav" aria-label="Settings sections">
                ${["general", "models", "appearance", "compaction"].map((id) => fe`<button class=${`settings-nav-item ${section === id ? "active" : ""}`} aria-current=${section === id ? "page" : undefined} onClick=${() => setSection(id)}>${{ general: "General", models: "Models", appearance: "Appearance", compaction: "Compaction" }[id]}</button>`)}
            </nav><main class="settings-content">
                ${section === "general" ? fe`<${General} />` : section === "appearance" ? fe`<${Appearance} />` : section === "compaction" ? fe`<${GiSettingsCompaction} key=${chatJid} chatJid=${chatJid} />` : fe`<${Models} key=${chatJid} chatJid=${chatJid} onMutationStart=${onMutationStart} onMutationEnd=${onMutationEnd} onApplied=${onApplied} />`}
            </main></div>
        </div>
    </div>`;
}
