      ...
      onClose();
        ...
        <div ref=${dialog} class="settings-dialog" role="dialog" aria-modal="true" aria-labelledby="gi-settings-title" onKeyDown=${(e) => e.stopPropagation()}>
            ...
            <div class="settings-dialog-body"><nav class="settings-nav" aria-label="Settings sections">
                ...
                ${section === "general" ? fe`<${General} />` : section === "appearance" ? fe`<${Appearance} />` : section === "providers" ? fe`<${GiSettingsProviders} />` : section === "compaction" ? fe`<${GiSettingsCompaction} key=${chatJid} chatJid=${chatJid} />` : fe`<${Models} key=${chatJid} chatJid=${chatJid} onMutationStart=${onMutationStart} onMutationEnd=${onMutationEnd} onApplied=${onApplied} />`}
            </main></div>
        </div>
    </div>`;
}
