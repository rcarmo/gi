...
class DraftConflictError extends Error {
  constructor() {
    super("Draft changed in another tab. Copy your unsaved text and attachments before reloading; sending is blocked in this tab.");
...
function indexedDraftStorage(factory = indexedDB) {
  ...
  const database = new Promise((resolve, reject) => {
    ...
    request.onblocked = () => {
      abandoned = true;
      reject(new Error("Draft database upgrade blocked by another tab. Close older Gi tabs, copy any unsaved edits, then reload."));
    };
  });
  return {
    async load() {
    ...
    async put(record, expectedRevision) {
      ...
      return new Promise((resolve, reject) => {
        ...
        current.onsuccess = () => {
          try {
            if (revision(current.result) !== expectedRevision)
              throw new DraftConflictError;
            if (expectedRevision === Number.MAX_SAFE_INTEGER)
              throw new Error("Draft revision exhausted; copy unsaved edits before recovery.");
    ...
    async load() {
      ...
      try {
        ...
        for (const row of rows) {
          ...
          revisions.set(row.sessionId, revision(row));
        }
      } catch (error) {
        loadFailure = error;
        records.clear();
...
// web/src/gi-recovery-control.ts
var PROTECTED_RECOVERY_CONTROL_INTENT = "protected_recovery_continuation";
var PROTECTED_RECOVERY_CONTROL_LABEL = "Recovery resumed with execution tools";
var PROTECTED_RECOVERY_REASONS = new Set([
...
var PROTECTED_RECOVERY_TYPED_KEYS = ["reason", "compaction", "tools_required", "retryable", "recovery_attempts"];
var PRIMARY_FAILURE_KEYS = [
...
function hasValidPrimaryFailureFields(block) {
  ...
  if (count === 0)
    return true;
  return count === PRIMARY_FAILURE_KEYS.length && block.primary_failure_category === "timeout" && typeof block.primary_failure_detail === "string" && block.primary_failure_detail.trim().length <= 500 && /^Timed out after \d+s\.$/.test(block.primary_failure_detail.trim()) && Number.isInteger(block.primary_failure_elapsed_ms) && block.primary_failure_elapsed_ms >= 0 && block.primary_failure_elapsed_ms <= 30 * 24 * 60 * 60 * 1000 && typeof block.primary_failure_execution_tools === "boolean" && typeof block.primary_failure_had_partial_output === "boolean" && typeof block.primary_failure_had_tool_activity === "boolean" && Number.isInteger(block.primary_failure_tool_executions) && block.primary_failure_tool_executions >= 0 && block.primary_failure_tool_executions <= 1e6;
}
function hasValidProtectedRecoveryHandoffFields(block) {
  ...
  if (!hasTypedFields)
    return hasValidPrimaryFailureFields(block);
  const valid = PROTECTED_RECOVERY_REASONS.has(block.reason) && ["not_attempted", "succeeded", "failed"].includes(block.compaction) && typeof block.tools_required === "boolean" && typeof block.retryable === "boolean" && Number.isInteger(block.recovery_attempts) && block.recovery_attempts >= 0;
  if (!valid || !hasValidPrimaryFailureFields(block))
    return false;
  if (block.reason === "post_compaction_tools_required")
    return block.compaction === "succeeded" && block.tools_required;
  if (block.reason === "compaction_failed")
    return block.compaction === "failed";
  if (block.reason === "tools_required" || block.reason === "unresolved_tool_execution")
    return block.tools_required;
  return true;
}
function getProtectedRecoveryControlIntent(contentBlocks) {
  const block = Array.isArray(contentBlocks) ? contentBlocks.find((candidate) => candidate && typeof candidate === "object" && candidate.type === "control_intent" && candidate.intent === PROTECTED_RECOVERY_CONTROL_INTENT && candidate.schema_version === 1 && typeof candidate.source_message_id === "string" && candidate.source_message_id.trim().length > 0 && Number.isInteger(candidate.source_row_id) && Number(candidate.source_row_id) > 0 && Number.isInteger(candidate.thread_id) && Number(candidate.thread_id) > 0 && Number.isInteger(candidate.handoff_depth ?? 1) && Number(candidate.handoff_depth ?? 1) > 0 && hasValidProtectedRecoveryHandoffFields(candidate)) : null;
  if (block) {
    return {
  ...
  return fe`
    ...
    if (tabStore.get(id))
      tabStore.closeOthers(id);
                        ...
                        onTogglePin=${(id) => tabStore.togglePin(id)}
                    />
