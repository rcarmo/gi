...
func TestChooseNextForkAgentID(t *testing.T) {
	...
	if got, ok := chooseNextForkAgentID(normalizeForkAgentBase("  "), map[string]bool{}); !ok || got != "agent1" {
		t.Fatalf("expected normalized default-base candidate agent1 for empty base, got %q ok=%v", got, ok)
	}
