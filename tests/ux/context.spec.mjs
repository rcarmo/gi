...
test('@ux-context-002 Keep unknown usage values visibly unknown',async({page,request},info)=>{
 ...
 await expect(meter).toHaveAttribute('title','Context: ? / ? tokens (?%) — usage unavailable');
 ...
 await page.reload();await expect(meter).toHaveAttribute('title','Context: ? / ? tokens (?%) — usage unavailable');
 ...
 await expect(input).toHaveValue('draft stays untouched');await expect(meter).toBeDisabled();
});
