...
test('guarded popup adaptations preserve rendering except one explicit Close button and reject source drift',()=>{
 ...
 const patched=patchQuickActionKeys(readFileSync('web/src/components/timeline-quick-actions.ts','utf8'));expect(patched.split(quickActionsCloseMarkup)).toHaveLength(2);expect(patched).toContain("event.target?.closest?.('button')");expect(patched).toContain('useLayoutEffect(() => {\n        const onKeyDown =');
