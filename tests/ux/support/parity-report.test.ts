...
test('parity report keeps shared evidence separate and requires every project',()=>{
 ...
 try{
  ...
  expect(all.counts.pass).toBe(2);expect(all.counts.unmapped).toBe(202);expect(all.sharedCounts.pass).toBe(2);
  ...
  expect(withMeter.counts.pass).toBe(4);expect(withMeter.counts.unmapped).toBe(202);expect(withMeter.sharedCounts.pass).toBe(2);
  ...
  expect(withCompact.counts.pass).toBe(10);expect(withCompact.counts.unmapped).toBe(202);
  ...
  expect(reconnected.counts.pass).toBe(4);expect(reconnected.counts.unmapped).toBe(202);
  ...
  expect(acknowledged.counts.pass).toBe(4);expect(acknowledged.counts.unmapped).toBe(202);
  expect(acknowledged.rows.find((row:any)=>row.id==='@ux-compose-008').status).toBe('unmapped');
