...
test('parity report keeps shared evidence separate and requires every project',()=>{
 ...
 try{
  ...
  expect(all.counts.pass).toBe(2);expect(all.counts.unmapped).toBe(207);expect(all.sharedCounts.pass).toBe(2);
  ...
  expect(withMeter.counts.pass).toBe(4);expect(withMeter.counts.unmapped).toBe(207);expect(withMeter.sharedCounts.pass).toBe(2);
  ...
  expect(withCompact.counts.pass).toBe(10);expect(withCompact.counts.unmapped).toBe(207);
  const reconnect=join(dir,'reconnect.json');writeFileSync(reconnect,JSON.stringify({suites:[{specs:[spec('@ux-reconnect-002',6),spec('@ux-reconnect-003',6),spec('@ux-reconnect-004',6)]}]}));
  ...
  expect(reconnected.counts.pass).toBe(3);expect(reconnected.counts.unmapped).toBe(207);
