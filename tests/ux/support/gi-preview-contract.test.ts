...
test('native preview lifecycle criteria do not expand the frozen corpus', () => {
    ...
    expect(cases).toHaveLength(2); expect(text).not.toContain('@ux-');
    expect(cases.map(row => row.tags.find(tag => tag.name.startsWith('@gi-preview-')).name)).toEqual(['@gi-preview-001','@gi-preview-002']);
