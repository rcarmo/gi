...
for(const mode of ['fullscreen','regular'])for(const [width,height]of [[60,18],[100,22],[140,36]]){
 ...
 try{
  ...
  await open();await wait(()=>capture().includes('× 2. opencode-zen/picker-small'),'disabled fit');const rows=capture().split('\n').filter(l=>/^\s*(?:›|\*|×)?\s*\d+\. /.test(l));assert(rows.length>0&&rows.length<=6,'selector bound');assert(rows.some(row=>row.includes('opencode-zen/picker-pine · 32K ctx')),'visible inline context');assert(rows.some(row=>row.includes('picker-small · context too small'))&&!rows.some(row=>row.includes('80 ctx')),'blocked reason precedes metadata');if(mode==='fullscreen')assert(bars().length===2,'additional idle separators');else assert(tmux('display-message','-p','-t',pane,'#{alternate_on}').trim()==='1','regular selector not isolated');
  ...
  await filter('32k ctx reasoning');assert(choice().includes('picker-pine · 32K ctx')&&!capture().includes('picker-piper'),'visible capability search');shot('filtered');
