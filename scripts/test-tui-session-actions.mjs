...
for(const mode of ['fullscreen','regular'])for(const [width,height] of [[60,18],[100,22],[140,36]]){
 ...
 try{
  ...
  const actionRows=cap().split('\n').filter(l=>/^\s*[›*×]?\s*\d+\. /.test(l));assert(actionRows.length<=3&&!actionRows.some(l=>l.includes('@')),'stale picker rows survived submenu shrink');shot('tall-to-short');keys('Escape');await wait(()=>cap().includes('Select session'),'restore tall picker');keys('Escape');
  ...
  keys('Right');await wait(()=>select('Restore'),'restore only');assert(!cap().includes('Unpin')&&!cap().split('\n').some(l=>/^\s*[› ]+\d+\. Pin\s*$/.test(l)),'archived edit advertised');shot('restore');keys('Enter');await wait(()=>sql(`select coalesce(json_extract(state_json,'$.archived_at'),'') from sessions where id='${child}';`)==='','restore');
  const originalTitle=sql(`select title from sessions where id='${child}';`);
  keys('Right');await wait(()=>cap().includes('Session actions'),'rename actions');keys('End');await wait(()=>select('Rename'),'rename choice');keys('Enter');await wait(()=>cap().includes('Rename session'),'rename prompt');
  assert(cap().includes('@actions-child'),'rename target opaque');keys('C-a','C-k');keys('Enter');await wait(()=>cap().includes('1–160'),'empty validation');assert(sql(`select title from sessions where id='${child}';`)===originalTitle,'invalid title wrote');
  type('Renamed 界 β');keys('Left');type('X');await sleep(120);assert(clean(cap()).includes('Renamed 界 Xβ'),'rename cursor not editable');keys('C-z');await sleep(120);assert(clean(cap()).includes('Renamed 界 β'),'rename undo failed');keys('End');
  tm('resize-window','-t','proof','-x','80','-y','24');await sleep(250);tm('resize-window','-t','proof','-x',String(width),'-y',String(height));await sleep(250);assert(clean(cap()).includes('Renamed 界 β'),'resize lost rename buffer');shot('rename');
  keys('Enter');await wait(()=>cap().includes('Select session'),'saved rename');assert(sql(`select title from sessions where id='${child}';`)==='Renamed 界 β','rename not durable');
  keys('Right');await wait(()=>cap().includes('Session actions'),'rename cancel actions');keys('End','Enter');await wait(()=>cap().includes('Rename session'),'cancel prompt');keys('C-a','C-k');type('Z'.repeat(160));await sleep(150);assert(cap().split('\n').filter(l=>l.includes('ZZZZ')).length===1,'long title added rows');keys('Home');type('Y');await sleep(80);keys('Escape');await wait(()=>cap().includes('Session actions'),'cancel to actions');assert(sql(`select title from sessions where id='${child}';`)==='Renamed 界 β','cancel wrote');keys('Escape');await wait(()=>cap().includes('Select session'),'cancel to picker');keys('Escape');await sleep(150);
  ...
  results.push({mode,width,height,pinUnpinArchiveRestore:true,renameSaveCancelValidation:true,rootBusyGates:true,draftCursorResize:true,zeroIdleRows:true,historyPreserved:true});
